...
export function TurmaDetail({ turma }: { turma: Turma; alunosIniciais: unknown[] }) {
  ...
  const cancelPublishedEdit = () => { setRows(data.students.map((student) => ({ id: student.id, fullName: student.nome, studentNumber: student.numero, specialStatus: student.specialStatus || "none" }))); setQuery(""); setCorrectionReason(""); setEditingPublished(false); };
  const addPublishedStudent = () => { setRows([...data.students.map((student) => ({ id: student.id, fullName: student.nome, studentNumber: student.numero, specialStatus: student.specialStatus || "none" as StudentSpecialStatus })), blank()]); setQuery(""); setCorrectionReason(""); setEditingPublished(true); };
  return <AppShell active="turmas" breadcrumb={turma.nome} currentClassId={turma.id}><Link className="back-link" href="/"><ArrowLeft />{t("classes.detail.back")}</Link><SurfaceHeader standalone headingLevel="h1" icon={<UsersRound />} eyebrow={t("classes.detail.academicYear")} title={turma.nome} actions={<div className="detail-heading__actions"><span className={`status ${isPublished ? "status--success" : "status--neutral"}`}>{isPublished ? t("classes.detail.published") : t("classes.detail.composition")}</span>{isPublished && data.permissions.edit && !editingPublished && <button type="button" className="button button--primary button--compact" onClick={addPublishedStudent}><Plus />{t("classes.detail.addPublished")}</button>}{isPublished && data.permissions.edit && <button type="button" className="button button--secondary button--compact" onClick={() => editingPublished ? cancelPublishedEdit() : setEditingPublished(true)}>{editingPublished ? <X /> : <PencilLine />}{editingPublished ? t("classes.detail.cancel") : t("classes.detail.managePublished")}</button>}</div>} />
    {notice && <AppToast key={`${noticeError ? "error" : "success"}:${notice}`} kind={noticeError ? "error" : "success"} message={notice} onDismiss={() => setNotice("")} />}
    {showEditor ? <section className="panel class-editor class-roster"><SurfaceHeader icon={<UsersRound />} eyebrow={isPublished ? t("classes.detail.admin") : t("classes.detail.composition")} title={isPublished ? t("classes.detail.editPublished") : t("classes.detail.students")} actions={<div className="roster-tools"><label className="search-field roster-search"><Search /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("classes.detail.searchPlaceholder")} /></label><div className="student-add-actions"><button className="student-add-primary" onClick={() => setRows((current) => [...current, blank()])}><Plus />{t("classes.detail.add")}</button><button className="student-add-batch" title={t("classes.detail.addFiveTitle")} aria-label={t("classes.detail.addFive")} onClick={() => setRows((current) => [...current, ...Array.from({ length: 5 }, blank)])}><UsersRound /><span>+5</span></button></div></div>} /><p className="surface-note">{isPublished ? t("classes.detail.editDescription") : t("classes.detail.composeDescription")}</p>
