...
function difficultyLabel(value: Difficulty) { return value === "easy" ? "Fáceis" : value === "hard" ? "Difíceis" : "Normais"; }
  ...
  return <AppShell active="quizzes_management" breadcrumb="Gestão de testes">
    <AdminPage>
      ...
      {section === "editor" && showEditor && <section className={`panel ${styles.editorPanel}`} aria-labelledby="editor-pergunta">
        ...
        <form className={styles.form} onSubmit={(event) => void saveQuestion(event)} noValidate>
          <div className={styles.formGrid}>
            ...
            <label><FormLabel icon={Gauge}>Dificuldade</FormLabel><select value={form.difficulty} onChange={(event) => update("difficulty", event.target.value as Difficulty)}><option value="easy">Fáceis</option><option value="medium">Normais</option><option value="hard">Difíceis</option></select></label>
      ...
      {section === "questions" && <section className={`panel ${styles.questionsPanel}`} aria-labelledby="perguntas-registadas">
        <SurfaceHeader icon={<CircleHelp />} eyebrow="Conteúdo" title="Perguntas registadas" headingId="perguntas-registadas" meta={`${pagination.total} no banco`} actions={<button className="button button--secondary button--compact" type="button" onClick={() => void load()} disabled={loading}><RefreshCw />Atualizar</button>} />
