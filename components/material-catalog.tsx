...
type ApiCard = { id: string; type: "multiple_choice" | "short_answer" | "image"; unitCode?: string; unitName?: string; lesson: string; subtopic: string; chapter?: string; question: string; answer?: string; hint?: string; source?: string; sourceLabel?: string; sourcePage?: string; sourceQuestion?: string; assessment?: string; session?: string; academicYear?: string; imageUrl?: string | null; tags?: string[]; options?: Array<{ text: string; isCorrect: boolean }> };
...
export function MaterialCatalog({ activeTab, onTabChange }: { activeTab: MaterialCatalogTab; onTabChange: (tab: MaterialCatalogTab) => void }) {
  ...
  const [items, setItems] = useState<CatalogItem[]>([]), [lessons, setLessons] = useState<Lesson[]>([]), [decks, setDecks] = useState<Deck[]>([]), [loading, setLoading] = useState(true), [error, setError] = useState(""), [catalogAttempt, setCatalogAttempt] = useState(0), [search, setSearch] = useState(""), [lessonFilter, setLessonFilter] = useState(""), [verificationFilter, setVerificationFilter] = useState<"all" | "original" | "verified" | "pending">("all"), [recommendedOnly, setRecommendedOnly] = useState(false), [ankiUnitCode, setAnkiUnitCode] = useState("NEURO"), [ankiVariant] = useState<"reviewed">("reviewed"), [ankiTypes, setAnkiTypes] = useState<Array<"multiple_choice" | "short_answer" | "image">>(["short_answer"]), [ankiLessons, setAnkiLessons] = useState<string[]>([]), [cards, setCards] = useState<ApiCard[]>([]), [cardsLoading, setCardsLoading] = useState(false), [cardsError, setCardsError] = useState(""), [ankiAttempt, setAnkiAttempt] = useState(0), [buildBusy, setBuildBusy] = useState(false), [downloadUrl, setDownloadUrl] = useState<string | null>(null), [notice, setNotice] = useState<Notice>(null);
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams({ variant: ankiVariant, unitId: selectedUnitId, unitCode: selectedCompendiumUnit.code }); ankiTypes.forEach((type) => params.append("type", type));
    fetch(`/api/material-anki?${params}`, { cache: "no-store", signal: controller.signal }).then(async (response) => {
      const data = await response.json() as { cards?: ApiCard[]; unit?: { code?: string; name?: string }; error?: string };
      ...
      if (!controller.signal.aborted) {
        ...
        const cardsForUnit = sourceCards.filter((card) => {
          const cardCode = card.unitCode || data.unit?.code;
          return cardCode ? cardCode.toLocaleUpperCase("pt-PT") === selectedCompendiumUnit.code : selectedCompendiumUnit.code === "NEURO";
        });
        setCards(cardsForUnit);
      }
  ...
  }, [activeTab, ankiAttempt, ankiLessons, ankiTypes, ankiVariant, selectedCompendiumUnit, selectedUnitId, t]);
  useEffect(() => () => { if (downloadUrl) URL.revokeObjectURL(downloadUrl); }, [downloadUrl]);
  useEffect(() => { setDownloadUrl((current) => { if (current) URL.revokeObjectURL(current); return null; }); }, [ankiLessons, ankiTypes, ankiUnitCode, ankiVariant]);
    ...
    <div id={`material-panel-${activeTab}`} role="tabpanel" aria-labelledby={`material-tab-${activeTab}`} tabIndex={-1}>
      {activeTab === "overview" && <>
        {loading ? <div className={styles.empty} role="status"><LoaderCircle className={styles.spin} /><span>{t("community.materials.catalog.loading")}</span></div> : error ? <div className={styles.empty} role="alert"><FileText /><strong>{t("community.materials.catalog.loadError")}</strong><button className="button button--ghost button--compact" type="button" onClick={retryCatalog}>{t("community.materials.catalog.retry")}</button></div> : <>
          ...
          <div className={styles.overviewNote}><Sparkles aria-hidden="true" /><div><strong>Materiais organizados para estudar por aula</strong><p>Os originais autorizados e as versões verificadas ficam identificados. Referências bibliográficas protegidas são mostradas apenas como metadados, sem distribuição de excertos.</p></div></div>
        </>}
      </>}
      ...
      {activeTab === "anki" && <div className={styles.ankiWorkspace}><div className={styles.ankiMain}><label className={styles.unitSelect}><span>Unidade curricular</span><select value={ankiUnitCode} onChange={(event) => { setAnkiUnitCode(event.target.value); setAnkiLessons([]); }}><optgroup label="2.º ano · 1.º semestre">{MATERIAL_COMPENDIUM_UNITS.filter((unit) => ["DECIDES I", "MP", "AR", "FIS1", "HIST1", "NEURO"].includes(unit.code)).map((unit) => <option key={unit.code} value={unit.code}>{unit.code} · {unit.title}</option>)}</optgroup><optgroup label="2.º ano · 2.º semestre">{MATERIAL_COMPENDIUM_UNITS.filter((unit) => ["FIS2", "DECIDES II", "HIST2", "IMUNO BAS", "PG"].includes(unit.code)).map((unit) => <option key={unit.code} value={unit.code}>{unit.code} · {unit.title}</option>)}</optgroup></select></label><div className={styles.overviewNote}><Check aria-hidden="true" /><div><strong>APKG revisto</strong><p>Gerado no navegador apenas a partir do banco moderado. Não inclui imagens de atlas, recortes de páginas nem os pacotes binários arquivados.</p></div></div><div className={styles.filterBlock}><span className={styles.controlLabel}>{t("community.materials.catalog.cardType")}</span><div className={styles.checkChips}>{(["short_answer"] as const).map((type) => <button key={type} type="button" aria-pressed={ankiTypes.includes(type)} className={ankiTypes.includes(type) ? styles.chipActive : ""} onClick={() => toggleType(type)}>{type === "multiple_choice" ? t("community.materials.catalog.multipleChoice") : type === "short_answer" ? t("community.materials.catalog.shortAnswer") : t("community.materials.catalog.image")}</button>)}</div></div>{cardsError && <div className={`${styles.notice} ${styles.noticeError}`} role="alert"><span>{cardsError}</span><button className="button button--ghost button--compact" type="button" onClick={retryAnki}>{t("community.materials.catalog.retry")}</button></div>}<div className={styles.builderPreview}><div><span className={styles.resourceLabel}>{t("community.materials.catalog.preview")}</span><h2>{selectedCompendiumUnit.shortTitle} · {t("community.materials.catalog.selectLessons")}</h2><p role="status">{cardsLoading ? t("community.materials.catalog.cardsLoading") : t("community.materials.catalog.cardsIncluded", { count: cards.length })}</p>{!ankiTypes.length && <p className={styles.selectionHint}>{t("community.materials.catalog.noCardTypes")}</p>}</div><div className={styles.cardTypeSummary}><span><Check />{cards.filter((card) => card.type === "multiple_choice").length} {t("community.materials.catalog.multipleChoice")}</span><span><Check />{cards.filter((card) => card.type === "short_answer").length} {t("community.materials.catalog.shortAnswer")}</span></div></div><button className="button button--primary" type="button" onClick={() => void generate()} disabled={buildBusy || cardsLoading || !cards.length || !ankiTypes.length}>{buildBusy ? <LoaderCircle className={styles.spin} /> : <Sparkles />}{buildBusy ? t("community.materials.catalog.creatingDeck") : t("community.materials.catalog.createDeck")}</button>{downloadUrl && <a className={`button button--secondary ${styles.downloadGenerated}`} href={downloadUrl} download={`${selectedCompendiumUnit.fileStem}_revisto_personalizado.apkg`}><Download />{t("community.materials.catalog.downloadCustomDeck")}</a>}<MaterialCompendiumExport cards={cards} unitId={selectedUnitId} unitCode={selectedCompendiumUnit.code} unitName={selectedCompendiumUnit.title} lessonOptions={lessonOptions} selectedLessons={ankiLessons} onToggleLesson={toggleLesson} selectedTypes={ankiTypes} onToggleType={toggleType} /></div><aside className={styles.ankiAside} aria-label={t("community.materials.catalog.recommended")}><span className={styles.overviewIcon} aria-hidden="true"><Package /></span><span className={styles.resourceLabel}>Versão segura</span><h2>{selectedCompendiumUnit.shortTitle} · Revisto</h2><p>Os pacotes APKG binários anteriores foram arquivados. Este baralho é criado localmente com perguntas publicadas após moderação e sem media bibliográfica protegida.</p><strong>{cards.length} {t("community.materials.catalog.cards")}</strong><span className={styles.pending}>Criação local, sem ficheiro pré-gerado.</span></aside></div>}
