...
export function MaterialCatalog({ activeTab, onTabChange }: { activeTab: MaterialCatalogTab; onTabChange: (tab: MaterialCatalogTab) => void }) {
  ...
  const [items, setItems] = useState<CatalogItem[]>([]), [lessons, setLessons] = useState<Lesson[]>([]), [decks, setDecks] = useState<Deck[]>([]), [loading, setLoading] = useState(true), [error, setError] = useState(""), [catalogAttempt, setCatalogAttempt] = useState(0), [search, setSearch] = useState(""), [lessonFilter, setLessonFilter] = useState(""), [verificationFilter, setVerificationFilter] = useState<"all" | "original" | "verified" | "pending">("all"), [recommendedOnly, setRecommendedOnly] = useState(false), [ankiUnitCode, setAnkiUnitCode] = useState("NEURO"), [ankiVariant, setAnkiVariant] = useState<"essential" | "complete">("essential"), [ankiTypes, setAnkiTypes] = useState<Array<"multiple_choice" | "short_answer" | "image">>(["short_answer", "image"]), [ankiLessons, setAnkiLessons] = useState<string[]>([]), [cards, setCards] = useState<ApiCard[]>([]), [cardsLoading, setCardsLoading] = useState(false), [cardsError, setCardsError] = useState(""), [ankiAttempt, setAnkiAttempt] = useState(0), [buildBusy, setBuildBusy] = useState(false), [downloadUrl, setDownloadUrl] = useState<string | null>(null), [notice, setNotice] = useState<Notice>(null);
  ...
  const selectedDeck = decks.find((deck) => deck.variant === ankiVariant && (!deck.unitId || deck.unitId === selectedUnitId));
  useEffect(() => {
    ...
    const params = new URLSearchParams({ variant: ankiVariant, unitId: selectedUnitId, unitCode: selectedCompendiumUnit.code }); ankiLessons.forEach((lesson) => params.append("lesson", lesson)); ankiTypes.forEach((type) => params.append("type", type));
