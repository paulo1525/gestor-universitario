...
export function TurmasDashboard() {
  ...
  useEffect(() => { void (async () => { try { const response = await fetch("/api/classes", { cache: "no-store" }); if (!response.ok) return; const data = await response.json() as { classes?: ApiClass[] }; setPlacementsPublished(Boolean(data.classes?.length && data.classes.every((item) => item.status === "published"))); setClasses((data.classes || []).map((item) => ({ id: item.id, nome: t("classes.common.class", { id: item.id }), representante: item.representative || t("classes.dashboard.unassigned"), alunos: Number(item.students), ficam: Number(item.stays || 0), mudam: Number(item.moves || 0), estado: labels[item.status] || "Em preenchimento" }))); } finally { setLoading(false); } })(); }, [t]);
