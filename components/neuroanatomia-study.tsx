...
export function StudyDocument() {
  ...
  return <div className={styles.page} data-reading-size={readingSize}>
    ...
    <dialog id="study-questions" ref={questionRef} className={styles.questions} data-app-modal-native aria-labelledby="study-questions-title" onClose={() => setSelection(null)} onClick={dismissOutside}>
      <header data-app-modal-header><strong id="study-questions-title">Perguntas do compêndio</strong><button className={styles.iconButton} type="button" aria-label="Fechar perguntas" onClick={closeQuestions}><X aria-hidden="true" /></button></header>
    ...
    <dialog ref={indexRef} className={styles.indexDialog} data-app-modal-native aria-labelledby="study-index-title" onClose={() => setIndexOpen(false)} onClick={dismissOutside}>
      <header className={styles.dialogHeader} data-app-modal-header><h2 id="study-index-title">Sumário</h2><button className={styles.iconButton} type="button" aria-label="Fechar sumário" onClick={() => indexRef.current?.close()}><X aria-hidden="true" /></button></header>
    ...
    {!notesOpen && deleteDialog}
    <dialog ref={dialogRef} className={styles.lightbox} data-app-modal-lightbox aria-labelledby="study-image-title" onClose={() => setLightboxImage(null)} onClick={dismissOutside}>
