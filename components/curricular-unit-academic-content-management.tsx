...
import { ArrowLeft, BadgeCheck, CalendarClock, ClipboardCheck, FileText, LoaderCircle, Plus, Save, ShieldAlert, Trash2 } from "lucide-react";
import { AppShell } from "@/components/app-shell";
import { AdminPage, AdminPageHeader, AdminSection, AdminFormGrid } from "@/components/admin-ui";
...
export function CurricularUnitAcademicContentManagement({ id }: { id: string }) {
  ...
  return <AppShell active="curricular_units_management" breadcrumb={unit?.name || "Conteúdo académico"}><AdminPage>
    <AdminPageHeader eyebrow="Conteúdo académico" title={unit ? `${unit.code} · ${unit.name}` : "Informação académica"} description="Mantém a descrição, as regras da unidade curricular e as fontes organizadas por ano letivo." actions={<div className={styles.pageActions}><label className={styles.yearPicker}><span>Ano letivo</span><select value={year} onChange={event => { setYear(event.target.value); void load(event.target.value); }}><option value={academicYearDefault()}>{academicYearDefault()}</option>{years.filter(item => item !== academicYearDefault()).map(item => <option value={item} key={item}>{item}</option>)}</select></label><Link className="button button--secondary button--compact" href="/admin/unidades-curriculares"><ArrowLeft />Voltar</Link></div>} />
    {notice && <AppToast kind={notice.kind} message={notice.message} onDismiss={() => setNotice(null)} />}
    <AdminSection icon={form.status === "verificado" ? <BadgeCheck /> : <ShieldAlert />} eyebrow="Estado da informação" title={statusLabel} description={form.status === "verificado" ? "O conteúdo foi confirmado pela Comissão de Curso." : "O conteúdo aparece aos estudantes com o aviso ‘A validar’."} actions={<span className={`${styles.status} ${form.status === "verificado" ? styles.statusVerified : styles.statusPending}`}>{form.status === "verificado" ? <BadgeCheck /> : <ShieldAlert />}{statusLabel}</span>}>
      <AdminFormGrid>
    ...
    <AdminSection icon={<ClipboardCheck />} eyebrow="Avaliação" title="Componentes de avaliação" description="Regista pesos, notas mínimas e regras relevantes para este ano letivo." actions={<button className="button button--secondary button--compact" type="button" onClick={addEvaluation}><Plus />Adicionar componente</button>}>
    ...
    <AdminSection icon={<CalendarClock />} eyebrow="Calendário académico" title="Frequências e exames" description="Liga cada época ao evento correspondente do calendário para manter datas e informação sincronizadas." actions={<button className="button button--secondary button--compact" type="button" onClick={addExam}><Plus />Adicionar exame</button>}>
    ...
    <AdminSection icon={<FileText />} eyebrow="Fontes" title="Fontes e bibliografia" description="Indica as fontes oficiais, regulamentos e bibliografia recomendada que sustentam a informação." actions={<button className="button button--secondary button--compact" type="button" onClick={addSource}><Plus />Adicionar fonte</button>}>
