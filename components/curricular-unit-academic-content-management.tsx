...
import { BadgeCheck, BookOpenText, CalendarClock, ClipboardCheck, FileText, LoaderCircle, Save, ShieldAlert, Trash2 } from "lucide-react";
...
type RowKey = "evaluations" | "exams" | "sources";

const FLOATING_EVALUATION_ICON = <ClipboardCheck aria-hidden="true" />;
const FLOATING_EXAM_ICON = <CalendarClock aria-hidden="true" />;
const FLOATING_SOURCE_ICON = <FileText aria-hidden="true" />;
const REMOVE_TITLES: Record<RowKey, string> = { evaluations: "Remover componente?", exams: "Remover exame?", sources: "Remover fonte?" };
...
export function CurricularUnitAcademicContentManagement({ id }: { id: string }) {
  ...
  const [removeTarget, setRemoveTarget] = useState<{ key: RowKey; index: number } | null>(null);
  ...
  const requestRemove = (key: RowKey, index: number) => {
    const item = form[key][index] as Record<string, string> | undefined;
    const filled = item && Object.entries(item).some(([field, value]) => field !== "examType" && field !== "sourceType" && value.trim());
    if (filled) setRemoveTarget({ key, index }); else removeArray(key, index);
  };
  const removeSubject = removeTarget ? (form[removeTarget.key][removeTarget.index] as { title?: string } | undefined)?.title?.trim() || undefined : undefined;

  useFloatingAction(!loading || unit ? { id: "add-evaluation", label: "Adicionar componente", icon: FLOATING_EVALUATION_ICON, onClick: addEvaluation } : null);
  useFloatingAction(!loading || unit ? { id: "add-exam", label: "Adicionar exame", icon: FLOATING_EXAM_ICON, onClick: addExam } : null);
  useFloatingAction(!loading || unit ? { id: "add-source", label: "Adicionar fonte", icon: FLOATING_SOURCE_ICON, onClick: addSource } : null);
  ...
  return <AppShell active="curricular_units_management" breadcrumb={unit?.name || "Conteúdo académico"}><AdminPage>
    <AdminPageHeader icon={<BookOpenText />} eyebrow="Conteúdo académico" title={unit ? `${unit.code} · ${unit.name}` : "Informação académica"} />
    ...
    <ConfirmationDialog open={Boolean(removeTarget)} eyebrow="" title={removeTarget ? REMOVE_TITLES[removeTarget.key] : ""} description="" subject={removeSubject} subjectLabel="Registo" confirmLabel="Remover" onClose={() => setRemoveTarget(null)} onConfirm={() => { if (removeTarget) removeArray(removeTarget.key, removeTarget.index); setRemoveTarget(null); }} />
  </AdminPage></AppShell>;
}
