...
export function AppShell({ children, active, breadcrumb = "Visão geral", currentClassId }: Props) {
  ...
  return <div className="app-shell">
    ...
    <div className="workspace"><header className="topbar"><button className="icon-button mobile-menu" onClick={() => setOpen(true)} aria-label="Abrir menu"><Menu /></button><div className="breadcrumbs"><Link href="/">Gestão de turmas</Link><ChevronRight /><strong>{breadcrumb}</strong></div></header><main id="conteudo-principal" className="main-content">{user?.role==="student"&&!user.classRepresentative&&!user.preview&&<StudentPreferencePanel/>}{children}</main></div>
