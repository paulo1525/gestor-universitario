  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${open ? "sidebar--open" : ""}`}>
      ...
      <nav className="nav-list" aria-label="Navegação principal">
        <div className="nav-section"><span className="nav-label">Turmas</span>{preferenceOnly ? <Link className={classesActive ? "is-active" : ""} href="/"><ClipboardCheck /><span><strong>Preferências</strong><small>Escolher ou manter turma</small></span></Link> : <Link className={classesActive ? "is-active" : ""} href="/"><Users /><span><strong>Lista de turmas</strong><small>Composição e submissões</small></span><span className="nav-count">{classCount}</span></Link>}{user?.representedClass && <Link className={ownActive ? "is-active" : ""} href={`/turmas/${user.representedClass}`}><Users /><span><strong>A minha turma</strong><small>Composição da Turma {user.representedClass}</small></span></Link>}{user?.role==="admin"&&<Link className={active === "placements"||active === "check" ? "is-active" : ""} href="/admin/colocacoes"><Grid3X3 /><span><strong>Colocações</strong><small>Validar, calcular e publicar</small></span></Link>}</div>
        {user?.role === "admin" && <div className="nav-section"><span className="nav-label">Administração</span><Link className={active === "admin" ? "is-active" : ""} href="/admin"><Settings /><span><strong>Configuração</strong><small>Utilizadores e calendário</small></span></Link><Link className={active === "audit" ? "is-active" : ""} href="/admin/historico"><History /><span><strong>Histórico</strong><small>Ações administrativas</small></span></Link></div>}
      </nav>
