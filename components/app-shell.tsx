...
import { BookOpen, Boxes, CalendarDays, Check, ChevronDown, ChevronRight, ChevronUp, ClipboardCheck, ContactRound, FileText, FlaskConical, Grid3X3, History, Inbox, Languages, LayoutDashboard, Library, LogOut, Megaphone, Menu, Palette, Settings, Users, Vote, X } from "lucide-react";
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${open ? "sidebar--open" : ""}`}>
      ...
      <nav className="nav-list" aria-label={t("shell.primaryNavigation")}>
        ...
        <div className="nav-section"><span className="nav-label">{t("nav.classes")}</span>{preferenceOnly ? moduleAccess["classes.preferences"]&&<Link className={classesActive ? "is-active" : ""} href="/"><ClipboardCheck /><span><strong>{t("nav.preferences.title")}</strong><small>{t("nav.preferences.description")}</small></span></Link> : moduleAccess["classes.rosters"]&&<Link className={classesActive ? "is-active" : ""} href="/"><Users /><span><strong>{t("nav.classList.title")}</strong><small>{t("nav.classList.description")}</small></span>{classCount !== null && <span className="nav-count">{classCount}</span>}</Link>}{moduleAccess["classes.rosters"]&&user?.representedClass && <Link className={ownActive ? "is-active" : ""} href={`/turmas/${user.representedClass}`}><Users /><span><strong>{t("nav.myClass.title")}</strong><small>{t("nav.myClass.description", { classId: user.representedClass })}</small></span></Link>}{moduleAccess["classes.placements"]&&user?.role==="admin"&&<Link className={active === "placements"||active === "check" ? "is-active" : ""} href="/admin/colocacoes"><Grid3X3 /><span><strong>{t("nav.placements.title")}</strong><small>{t("nav.placements.description")}</small></span></Link>}</div>
