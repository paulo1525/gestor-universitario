...
export function AppShell({ children, active, breadcrumb = "Visão geral", currentClassId }: Props) {
  ...
  useEffect(()=>{const controller=new AbortController();fetch("/api/classes",{cache:"no-store",signal:controller.signal}).then(async response=>await response.json() as {classes?:unknown[]}).then(data=>setClassCount(data.classes?.length||20)).catch(()=>{});return()=>controller.abort()},[]);
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${open ? "sidebar--open" : ""}`}>
      ...
      <nav className="nav-list" aria-label="Navegação principal">
        <div className="nav-section"><span className="nav-label">Gestão de turmas</span><Link className={classesActive ? "is-active" : ""} href="/"><Users />Turmas<span className="nav-count">{classCount}</span></Link>{user?.representedClass && <Link className={ownActive ? "is-active" : ""} href={`/turmas/${user.representedClass}`}><Users />A minha turma<span className="nav-count">{user.representedClass}</span></Link>}</div>
    ...
    <div className="workspace"><header className="topbar"><button className="icon-button mobile-menu" onClick={() => setOpen(true)} aria-label="Abrir menu"><Menu /></button><div className="breadcrumbs"><Link href="/">Gestão de turmas</Link><ChevronRight /><strong>{breadcrumb}</strong></div></header><main id="conteudo-principal" className="main-content">{active!=="overview"&&user?.role==="student"&&!user.classRepresentative&&!user.preview&&<StudentPreferencePanel/>}{children}</main></div>
