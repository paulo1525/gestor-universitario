...
import { Beaker, ChevronRight, ClipboardCheck, Grid3X3, History, LogOut, Menu, Settings, Ticket, Users, X } from "lucide-react";
import { ReactNode, useEffect, useState } from "react";
import { FontScale, useAuth } from "@/components/auth-context";

type Props = { children: ReactNode; active: "overview" | "turmas" | "admin" | "tickets" | "check" | "placements" | "audit" | "testing"; breadcrumb?: string; currentClassId?: number };
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${open ? "sidebar--open" : ""}`}>
      ...
      <nav className="nav-list" aria-label="Navegação principal">
        ...
        {user?.role === "admin" && <div className="nav-section"><span className="nav-label">Gestão administrativa</span><Link className={active === "admin" ? "is-active" : ""} href="/admin"><Settings />Controlo administrativo</Link><Link className={active === "testing" ? "is-active" : ""} href="/admin/teste"><Beaker />Ambiente de testes</Link><Link className={active === "tickets" ? "is-active" : ""} href="/admin/pedidos"><Ticket />Tickets</Link><Link className={active === "check" ? "is-active" : ""} href="/admin/verificacao"><ClipboardCheck />Verificador</Link><Link className={active === "placements" ? "is-active" : ""} href="/admin/colocacoes"><Grid3X3 />Colocações</Link><Link className={active === "audit" ? "is-active" : ""} href="/admin/historico"><History />Histórico de ações</Link></div>}
      </nav>
    ...
    <div className="workspace"><header className="topbar"><button className="icon-button mobile-menu" onClick={() => setOpen(true)} aria-label="Abrir menu"><Menu /></button><div className="breadcrumbs">{preferenceOnly ? <strong>As minhas preferências</strong> : <><Link href="/">Gestão de turmas</Link><ChevronRight /><strong>{breadcrumb}</strong></>}</div>{user?.preview && <button className="preview-user-toggle" onClick={() => void stopPreview()}><EyeLabel/><span><small>A visualizar como</small><strong>{user.fullName}</strong></span><b>Voltar ao meu perfil</b></button>}</header><main id="conteudo-principal" className={`main-content${preferenceOnly ? " main-content--preference" : ""}`}>{children}</main></div>
  </div>;
}
