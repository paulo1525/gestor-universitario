...
import { AlertTriangle, Check, Clipboard, FileSpreadsheet, LoaderCircle, Sparkles, Upload, X } from "lucide-react";
    ...
    <section className={`panel ${styles.panel}`}>
      <header className={styles.header}>
        ...
        <div className={styles.headerCopy}>
          <span className="eyebrow">{t("classes.import.eyebrow")}</span>
          <h2>{t("classes.import.title")}</h2>
          <p>{t("classes.import.description")}</p>
        </div>
        ...
        <div className={styles.uploadColumn}>
          ...
          <div className={styles.actionRow}>
            <div className={styles.actionStatus} aria-live="polite">
              {students.length > 0
                ? <><Check /><span><strong>{t("classes.import.ready", { count: students.length })}</strong><small>{t("classes.import.readyClasses", { count: new Set(students.map((student) => student.turma)).size })}</small></span></>
                : <><span className={styles.statusDot} aria-hidden="true" /><small>{t("classes.import.additive")}</small></>}
            </div>
            <button type="button" className="button button--primary button--compact" disabled={!file || !students.length || importing} onClick={() => void importCsv()}>{importing ? <LoaderCircle className={styles.spinner} /> : <Upload />}{importing ? t("classes.import.importingShort") : t("classes.import.action")}</button>
          </div>
        </div>
