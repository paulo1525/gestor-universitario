...
export function ClassRosterImport({ onImported }: { onImported?: () => void | Promise<void> }) {
  ...
  return <>
    ...
    {showPrompt && <div className={styles.modalBackdrop} data-app-modal-backdrop role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setShowPrompt(false); }}><section className={styles.modal} data-app-modal="modal" data-app-modal-size="medium" role="dialog" aria-modal="true" aria-labelledby="csv-ai-prompt-title"><header data-app-modal-header><div><span className="eyebrow">{t("classes.import.aiEyebrow")}</span><h2 id="csv-ai-prompt-title">{t("classes.import.aiTitle")}</h2></div><button type="button" data-app-modal-close onClick={() => setShowPrompt(false)} aria-label={t("classes.import.close")}><X /></button></header><p>{t("classes.import.aiDescription")}</p><pre>{AI_PROMPT}</pre><footer data-app-modal-footer><button type="button" className="button button--primary" data-app-modal-action="primary" onClick={() => void copyPrompt()}>{copied ? <Check /> : <Clipboard />}{copied ? t("classes.import.copied") : t("classes.import.copy")}</button></footer></section></div>}
  </>;
}
