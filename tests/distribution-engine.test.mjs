...
test("colegas só favorecem destinos escolhidos em comum",()=>{const rows=balanced().map(row=>row.id==="base-2-0"?{...row,preference:"move",destinations:[3,2]}:row);rows.push({...student("amigo",1,[4,3]),friendPreferences:[{friendStudentId:"base-2-0",destinations:[3,2],rank:1}]});const result=calculateDistribution(rows,{seed:"amigo"}).find(row=>row.studentId==="amigo");assert.equal(result.destinationClass,3)});
