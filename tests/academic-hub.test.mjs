...
test("pedidos e materiais suportam anonimato sem expor normalmente a identidade", () => {
  ...
  assert.match(requests, /Enviar anonimamente/);
  assert.match(requests, /Enviar com identificação/);
  assert.match(materials, /Enviar anonimamente/);
  assert.match(materials, /moderação/);
...
test("os votos são anónimos, impedem duplicados e validam opções", () => {
  ...
  assert.match(polls, /Voto anónimo/);
});
...
test("diretório e áreas de UC usam users e agregam informação académica", () => {
  ...
  assert.match(directory, /sincronizada com os perfis (?:de utilizador|institucionais)/);
  assert.match(unitCatalog, /Próximos momentos/);
  assert.match(unitCatalog, /Documentos e materiais/);
});
...
test("calendar supports optimistic drag-and-drop rescheduling with an accessible fallback", () => {
  ...
  assert.match(calendar, /A alteração foi revertida/);
});
