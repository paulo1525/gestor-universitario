...
test("gestão de utilizadores adapta filtros e registos ao ecrã mobile",()=>{
  ...
  assert.match(styles,/\.admin-table-wrap tbody tr\s*\{[\s\S]*?display:\s*flex[\s\S]*?flex-direction:\s*column/);
  assert.match(styles,/\.admin-table-wrap tbody tr\s*\{[\s\S]*?overflow:\s*hidden/);
  assert.match(styles,/\.admin-table-wrap tbody td:last-child\s*\{[\s\S]*?display:\s*flex[\s\S]*?padding-bottom:\s*14px/);
