...
function excelXml(value:unknown){return String(value??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\"/g,"&quot;")}
function testExcel(rows:TestStudent[],decisions:Record<string,number[]>){
 const cell=(value:unknown,style="Default")=>`<Cell ss:StyleID="${style}"><Data ss:Type="String">${excelXml(value)}</Data></Cell>`;
 const headers=["Nome","Número mecanográfico","Turma atual","Decisão","Preferências"];
 const body=rows.map(row=>`<Row>${cell(row.full_name)}${cell(row.student_number)}${cell(`Turma ${row.class_id}`)}${cell(row.student_decision==="move"?"Mudar":row.student_decision==="stay"?"Ficar":"Sem decisão")}${cell((decisions[row.id]||[]).map(id=>`Turma ${id}`).join(" → "))}</Row>`).join("");
 return `<?xml version="1.0" encoding="UTF-8"?><?mso-application progid="Excel.Sheet"?><Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet" xmlns:x="urn:schemas-microsoft-com:office:excel"><Styles><Style ss:ID="Default"><Alignment ss:Vertical="Center"/><Font ss:FontName="Aptos" ss:Size="10"/><Borders><Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#E6E2D8"/></Borders></Style><Style ss:ID="Title"><Font ss:Bold="1" ss:Size="16" ss:Color="#FFFFFF"/><Interior ss:Color="#171714" ss:Pattern="Solid"/></Style><Style ss:ID="Header"><Font ss:Bold="1" ss:Color="#171714"/><Interior ss:Color="#F6C945" ss:Pattern="Solid"/><Alignment ss:Vertical="Center"/></Style></Styles><Worksheet ss:Name="Colocações"><Table><Column ss:Width="190"/><Column ss:Width="110"/><Column ss:Width="90"/><Column ss:Width="90"/><Column ss:Width="260"/><Row ss:Height="30">${cell("Gestor Universitário — ambiente de testes","Title")}<Cell ss:MergeAcross="3" ss:StyleID="Title"><Data ss:Type="String">Dados exclusivamente fictícios</Data></Cell></Row><Row ss:Height="24">${headers.map(value=>cell(value,"Header")).join("")}</Row>${body}</Table><AutoFilter x:Range="R2C1:R${rows.length+2}C5"/><WorksheetOptions><FreezePanes/><FrozenNoSplit/><SplitHorizontal>2</SplitHorizontal><TopRowBottomPane>2</TopRowBottomPane></WorksheetOptions></Worksheet></Workbook>`;
}
...
async function mock(input:RequestInfo|URL,init?:RequestInit):Promise<Response|null>{const raw=typeof input==="string"?input:input instanceof URL?input.pathname:input.url,url=new URL(raw,location.origin),path=url.pathname,method=(init?.method||"GET").toUpperCase(),s=state(),r=s.roster||roster,persona=TEST_PERSONAS.find(item=>item.id===testPersona())||TEST_PERSONAS[0];
 ...
 if(path==="/api/classes"&&method==="GET")return reply({classes:Array.from({length:5},(_,i)=>({id:i+1,status:s.proposalStatus==="published"?"published":i===4?"draft":"submitted",submitted_at:i===4&&s.proposalStatus!=="published"?null:Date.now(),representative:"Comissão de Curso",students:r.filter(row=>row.class_id===i+1).length,stays:r.filter(row=>row.class_id===i+1&&row.student_decision==="stay").length,moves:r.filter(row=>row.class_id===i+1&&row.student_decision==="move").length}))});
 ...
 if(path==="/api/admin/export-validation"&&method==="GET")return new Response(testExcel(r,s.decisions),{headers:{"content-type":"application/vnd.ms-excel; charset=utf-8","content-disposition":"attachment; filename=colocacoes-ambiente-teste.xls"}})
 ...
 if(/^\/api\/admin\/distribution-proposals\//.test(path)&&method==="POST"){const action=path.split("/").pop();if(action!=="calculate"&&!s.proposalStatus)return reply({error:"Calcula primeiro uma proposta."},409);if(action==="apply")s.roster=r.map(row=>row.student_decision==="move"&&s.decisions[row.id]?.[0]?{...row,class_id:s.decisions[row.id][0]}:row);if(action==="rollback")s.roster=structuredClone(roster);s.proposalStatus=action==="calculate"?"draft":action==="approve"?"approved":action==="apply"?"applied":action==="publish"?"published":action==="rollback"?null:s.proposalStatus;saveState(s);return reply(action==="calculate"?{ok:true,status:"draft",proposal:{id:"test-proposal"}}:{ok:true,status:s.proposalStatus})}
 return null}
