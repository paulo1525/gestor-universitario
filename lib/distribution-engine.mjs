...
export function calculateDistribution(students, { seed = "distribution", maxDifference = 3, classIds } = {}) {
  ...
  for (const student of students) if (student.friendGroupCode) groups.set(student.friendGroupCode,[...(groups.get(student.friendGroupCode)||[]),student]);
  const groupTargets = new Map();
  for (const [code,members] of groups) {
    if (members.length < 2) continue;
    const common = members[0].destinations.filter(id=>members.every(member=>member.destinations.includes(id)));
    if (common.length) common.sort((a,b)=>members.reduce((sum,member)=>sum+member.destinations.indexOf(a),0)-members.reduce((sum,member)=>sum+member.destinations.indexOf(b),0));
    if (common[0]) groupTargets.set(code,common[0]);
  }
  const ordered = students.map(student => {
    const friendClasses=(student.friendPreferences||[]).sort((a,b)=>a.rank-b.rank).map(friend=>friend.classId);
    const preferred=[groupTargets.get(student.friendGroupCode),student.supportClass,...friendClasses,...student.destinations].filter((id,index,list)=>Number.isInteger(id)&&list.indexOf(id)===index);
