...
function solveForMinimum(students, movers, fixedCounts, activeClasses, minimum, maximum, seed, tieDirection = 1, forbidden = null) {
  const source = 0;
  const moverOffset = 1;
  ...
  for (let index = 0; index < movers.length; index += 1) {
    ...
    for (const classId of choices) {
      if (forbidden?.studentId === student.id && forbidden.classId === classId) continue;
      addEdge(graph, node, classNode.get(classId), 1, placementCost(student, classId, seed, activeClasses.length, movers.length, tieDirection), { studentId: student.id, classId });
    }
  }
...
export function calculateDistribution(students, { seed = "distribution", maxDifference = 3, classIds, objective = "preferences" } = {}) {
  ...
  const findBest = (allowedDifference, tieDirection, forbidden = null) => {
    ...
    for (let minimum = minimumStart; minimum <= averageFloor; minimum += 1) {
      const candidate = solveForMinimum(normalized, movers, fixedCounts, activeClasses, minimum, minimum + allowedDifference, seed, tieDirection, forbidden);
  ...
  for (const student of movers) {
    if (randomized.has(student.id)) continue;
    const alternative = findBest(minimumDifference, 0, { studentId: student.id, classId: best.assignment.get(student.id) });
    for (const studentId of randomizedStudents(movers, best, alternative)) randomized.add(studentId);
  }
