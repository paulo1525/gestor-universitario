...
const RANK_WEIGHT = 100_000_000_000;
const POINT_WEIGHT = 5_000_000_000;
const LOWER_SLOT_REWARD = 800_000_000_000_000;

function placementCost(student, destination, seed) {
  const rank = student.destinations.indexOf(destination);
  if (rank < 0) return (student.destinations.length + 1) * RANK_WEIGHT + tie(seed, student.id);
  const points = Math.max(0, Math.min(5, Number(student.basePoints) || 0));
  return rank * RANK_WEIGHT - points * POINT_WEIGHT + tie(seed, student.id);
}
...
function solveForMinimum(students, movers, fixedCounts, activeClasses, minimum, maximum, seed) {
  ...
  for (let index = 0; index < movers.length; index += 1) {
    ...
    for (const classId of choices) addEdge(graph, node, classNode.get(classId), 1, placementCost(student, classId, seed), { studentId: student.id, classId });
  }
  ...
  const objective = movers.reduce((total, student) => total + placementCost(student, assignment.get(student.id), seed), 0);
...
export function calculateDistribution(students, { seed = "distribution", maxDifference = 3, classIds } = {}) {
  ...
  const movers = normalized.filter(canStudentMove), fixedCounts = new Map(activeClasses.map(classId => [classId, 0]));
