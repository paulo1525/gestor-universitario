...
function placementCost(student, destination, seed, preferenceLevels, objective, moverCount) {
  ...
  if (rank < 0) {
    ...
    const stayPenalty = objective === "maximize_moves" ? (moverCount + 1) * maximumPreferenceSwing : 0;
...
function solveForMinimum(students, movers, fixedCounts, activeClasses, minimum, maximum, seed, objective) {
  const source = 0;
  const moverOffset = 1;
  ...
  for (let index = 0; index < movers.length; index += 1) {
    ...
    for (const classId of choices) addEdge(graph, node, classNode.get(classId), 1, placementCost(student, classId, seed, activeClasses.length, objective, movers.length), { studentId: student.id, classId });
  }
  ...
  const objectiveValue = movers.reduce((total, student) => total + BigInt(placementCost(student, assignment.get(student.id), seed, activeClasses.length, objective, movers.length)), 0n);
  ...
  return { assignment, counts, objective: objectiveValue, moved, difference, signature };
}
...
export function calculateDistribution(students, { seed = "distribution", maxDifference = 3, classIds, objective = "preferences" } = {}) {
  ...
  for (let minimum = minimumStart; minimum <= averageFloor; minimum += 1) {
    const candidate = solveForMinimum(normalized, movers, fixedCounts, activeClasses, minimum, minimum + maxDifference, seed, objective);
    const betterForMaximumMoves = candidate && objective === "maximize_moves" && (!best || candidate.moved > best.moved || (candidate.moved === best.moved && (candidate.difference < best.difference || (candidate.difference === best.difference && (candidate.objective < best.objective || (candidate.objective === best.objective && candidate.signature < best.signature))))));
    const betterForPreferences = candidate && objective === "preferences" && (!best || candidate.objective < best.objective || (candidate.objective === best.objective && (candidate.difference < best.difference || (candidate.difference === best.difference && candidate.signature < best.signature))));
    if (betterForMaximumMoves || betterForPreferences) best = candidate;
  }
