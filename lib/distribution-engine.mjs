...
function friendPriority(student, destination) {
  const ranks = (student?.friendPreferences || []).filter(friend => [friend.friendClassId, ...(friend.destinations || [])].includes(destination)).map(friend => friend.rank);
...
function comparePlacementPriority(first, second, destination) {
  ...
  return (firstRank < 0 ? Number.MAX_SAFE_INTEGER : firstRank) - (secondRank < 0 ? Number.MAX_SAFE_INTEGER : secondRank) || friendPriority(first, destination) - friendPriority(second, destination);
}
...
export function calculateDistribution(students, { seed = "distribution", maxDifference = 3, classIds } = {}) {
  const ordered = students.map(student => {
    const shared=(student.friendPreferences||[]).flatMap(friend=>{const friendOptions=[friend.friendClassId,...(friend.destinations||[])].filter((id,index,list)=>Number.isInteger(id)&&list.indexOf(id)===index);return student.destinations.filter(classId=>friendOptions.includes(classId)).map(classId=>({classId,worst:Math.max(student.destinations.indexOf(classId),friendOptions.indexOf(classId)),sum:student.destinations.indexOf(classId)+friendOptions.indexOf(classId),rank:friend.rank}));}).sort((a,b)=>a.worst-b.worst||a.sum-b.sum||a.rank-b.rank||tie(seed,`${student.id}:${a.classId}`)-tie(seed,`${student.id}:${b.classId}`)).map(item=>item.classId);
