...
export function calculateDistribution(students, { seed = "distribution", maxDifference = 3, classIds } = {}) {
  // Casos com menos alternativas são tratados primeiro para reduzir bloqueios globais;
  // critérios validados pela CC só desempatem casos com a mesma flexibilidade.
  const ordered = students.map(student => ({ ...student, destinations: [...student.destinations] }))
    .sort((a, b) => a.destinations.length - b.destinations.length || (b.basePoints || 0) - (a.basePoints || 0) || tie(seed, a.id) - tie(seed, b.id));
