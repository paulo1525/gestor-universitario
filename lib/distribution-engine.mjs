...
export function calculateDistribution(students, { seed = "distribution", maxDifference = 3 } = {}) {
  ...
  const occupants = new Map(Array.from({ length: 20 }, (_, index) => [index + 1, new Set()]));
  for (const student of ordered) occupants.get(student.classId).add(student.id);
  ...
  return ordered.map(student => {
    ...
    return { studentId: student.id, originClass: student.classId, destinationClass, rank: rank || null, status: student.preference === "stay" ? "stayed_by_choice" : destinationClass === student.classId ? "fallback" : "moved", randomized: randomized.has(student.id), manualReview: Boolean(student.notes?.trim()) };
  });
}
