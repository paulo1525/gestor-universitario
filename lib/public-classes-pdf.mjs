...
function pageStream({classId,students,publishedAt,pageNumber,pageCount,continuation,documentLabel,dateLabel,footer}){
  ...
  stream+=text("F1",9,"0.83 0.83 0.79",42,779,`Ano letivo 2026/2027 | ${documentLabel}`);
  stream+=text("F2",22,"0.10 0.10 0.09",42,712,`Turma ${classId}${continuation?" - continuação":""}`);
  stream+=text("F1",9,"0.37 0.36 0.32",42,692,`${students.length} ${students.length===1?"estudante nesta página":"estudantes nesta página"} | ${dateLabel} ${publishedAt}`);
  ...
  stream+=text("F1",8,"0.38 0.37 0.33",42,35,footer);
  stream+=text("F1",8,"0.38 0.37 0.33",486,35,`Página ${pageNumber} de ${pageCount}`);
  return latin1(stream);
}

export function buildPublicClassesPdf({classes,students,publishedAt,documentLabel="Documento público",dateLabel="Publicado em",footer="Inclui apenas nome, número mecanográfico e turma final publicada."}){
  ...
  pageSpecs.forEach((spec,index)=>{const pageObjectId=5+index*2,contentObjectId=pageObjectId+1,stream=pageStream({...spec,publishedAt,documentLabel,dateLabel,footer,pageNumber:index+1,pageCount});pageObjectIds.push(pageObjectId);objects.set(pageObjectId,latin1(`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 595 842] /Resources << /Font << /F1 3 0 R /F2 4 0 R >> >> /Contents ${contentObjectId} 0 R >>`));objects.set(contentObjectId,concatBytes([latin1(`<< /Length ${stream.length} >>\nstream\n`),stream,latin1("endstream")]))});
