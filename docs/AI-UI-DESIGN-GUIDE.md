...
- Páginas de biblioteca ou listagem (Avisos, Pedidos, Materiais, Documentos…) usam a mesma anatomia (referência: `components/requests-center.tsx` e `components/record-list.tsx`): cabeçalho da página; um único painel sem cabeçalho de secção repetido, com a `FilterBar` (incluindo pesquisa) no topo; uma linha por registo com título clicável e uma linha de metadados; ao abrir, cartão de leitura em `#<prefixo>-<id>` com botão de voltar; ações do item no menu flutuante; paginação partilhada (`components/pagination.tsx`) nas listas longas. Em Materiais, a unidade curricular escolhe-se primeiro. Não criar barras de contexto, cartões de resumo ou painéis adicionais por módulo.
