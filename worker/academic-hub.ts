...
async function unitCatalog(env: HubEnv, user: HubUser | null, enabled: ModuleChecker): Promise<Response> {
  ...
  const [result, representativesResult] = await Promise.all([
    ...
    env.DB.prepare("SELECT cur.curricular_unit_id,cur.user_id,cur.position,u.full_name,u.email,p.label AS position_label,d.label AS department_label FROM curricular_unit_representatives cur JOIN curricular_units cu ON cu.id=cur.curricular_unit_id JOIN users u ON u.id=cur.user_id LEFT JOIN commission_positions p ON p.code=u.commission_position LEFT JOIN commission_departments d ON d.code=u.commission_department WHERE cu.active=1 ORDER BY cur.curricular_unit_id,cur.position").all(),
  ]);
  const representativeMap = new Map<string, Array<{ id:unknown;name:unknown;email:unknown;position:unknown;department:unknown }>>();
  for (const item of representativesResult.results) {
    const representative = rowObject(item), list = representativeMap.get(String(representative.curricular_unit_id)) || [];
    list.push({ id: representative.user_id, name: representative.full_name, email: representative.email, position: representative.position_label, department: representative.department_label });
    representativeMap.set(String(representative.curricular_unit_id), list);
  }
...
async function unitDetail(env: HubEnv, id: string, user: HubUser | null, enabled: ModuleChecker, requestedAcademicYear = ""): Promise<Response> {
  ...
  const [representativesResult, facultyResult, eventsResult, docsResult, announcementsResult, materialsResult, academicContent] = await Promise.all([
    ...
    env.DB.prepare("SELECT f.id,f.full_name,f.title,f.email,f.office FROM campus_faculty_units cfu JOIN campus_faculty f ON f.id=cfu.faculty_id WHERE cfu.curricular_unit_id=? AND f.active=1 ORDER BY f.full_name COLLATE NOCASE").bind(id).all(),
...
async function personalDashboard(env: HubEnv, user: HubUser | null, enabled: ModuleChecker): Promise<Response> {
  ...
  const [upcomingResult, requestsResult, favoritesResult, urgentResult, pollsResult, completedQuizAttemptsResult, preferences, classInfo] = await Promise.all([
    ...
    env.DB.prepare("SELECT COUNT(*) AS n FROM quiz_attempts WHERE user_id=? AND completed_at IS NOT NULL").bind(user.id).first<{ n: number }>(),
    preferencesPromise,
  ...
  const completedQuizAttempts = completedQuizAttemptsResult?.n || 0;
  // PDFs being studied: the reader's private highlights, most recent first.
  const readingEnabled = await enabled("materials.catalog") && await enabled("materials.library");
  const readingResult = readingEnabled
    ? await env.DB.prepare("SELECT h.material_id,COUNT(*) AS highlight_count,MAX(COALESCE(h.updated_at,h.created_at)) AS last_activity,m.title,m.material_kind,cu.code AS unit_code,cu.name AS unit_name FROM material_pdf_highlights h JOIN material_catalog m ON m.id=h.material_id AND m.publication_status='published' LEFT JOIN curricular_units cu ON cu.id=m.curricular_unit_id WHERE h.user_id=? GROUP BY h.material_id ORDER BY last_activity DESC LIMIT 4").bind(user.id).all().catch(() => ({ results: [] as unknown[] }))
    : { results: [] as unknown[] };
