...
async function handleOwnDestinations(request:Request,env:Env,user:CurrentUser):Promise<Response>{
  ...
  const friends=Array.isArray(body?.friends)?body.friends.map((item)=>({studentId:String(item?.studentId||""),destinationClass:Number(item?.destinationClass)})):[];if(friends.length>3||new Set(friends.map(item=>item.studentId)).size!==friends.length||friends.some(item=>!item.studentId||item.studentId===student.id||!destinations.includes(item.destinationClass)))return json({error:"Pode associar até três colegas distintos, todos pertencentes a uma turma indicada nas preferências."},400);
