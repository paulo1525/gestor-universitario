...
async function handleClassesV2(request: Request, env: Env, user: CurrentUser, pathname: string): Promise<Response> {
  ...
  if(request.method==="PUT"&&action==="save"){
   ...
   const before=await env.DB.prepare("SELECT id,full_name,student_number FROM class_students WHERE class_id=? AND removed_at IS NULL ORDER BY student_number").bind(classId).all<{id:string;full_name:string;student_number:string}>(),now=Date.now(),actorId=user.actorId||user.id,values=normalized.students.map(()=>"(?,?,?,?,?,?,?,?,?,NULL)").join(","),bindings=normalized.students.flatMap(student=>[student.id,classId,student.fullName,student.studentNumber,student.preference,now,actorId,now,now]),publishedCorrection=klass.status==="published";
   await env.DB.batch([env.DB.prepare("UPDATE class_students SET removed_at=?,updated_at=? WHERE class_id=? AND removed_at IS NULL AND student_number NOT IN ("+normalized.students.map(()=>"?").join(",")+")").bind(now,now,classId,...normalized.students.map(student=>student.studentNumber)),env.DB.prepare(`INSERT INTO class_students (id,class_id,full_name,student_number,preference,preference_locked_at,created_by,created_at,updated_at,removed_at) VALUES ${values} ON CONFLICT(student_number) DO UPDATE SET class_id=excluded.class_id,full_name=excluded.full_name,updated_at=excluded.updated_at,removed_at=NULL`).bind(...bindings),env.DB.prepare("UPDATE classes SET status=CASE WHEN status='published' THEN 'published' ELSE 'submitted' END,workflow_step=1,submitted_at=?,submitted_by=?,updated_at=? WHERE id=?").bind(now,actorId,now,classId),env.DB.prepare("INSERT INTO class_audit_log (class_id,actor_user_id,action,details,created_at) VALUES (?,?,?,?,?)").bind(classId,actorId,publishedCorrection?"published_roster_corrected":"class_roster_saved",JSON.stringify({reason:reason||null,before:before.results,after:normalized.students.map(student=>({id:student.id,fullName:student.fullName,studentNumber:student.studentNumber})),students:normalized.students.length}),now),env.DB.prepare("INSERT INTO admin_audit_log (actor_user_id,action,details,created_at) SELECT ?,'published_roster_corrected',?,? WHERE ?=1").bind(actorId,JSON.stringify({classId,reason,before:before.results,after:normalized.students}),now,publishedCorrection?1:0)]);
    return json({ok:true});
  }
...
async function handleOwnDestinations(request:Request,env:Env,user:CurrentUser):Promise<Response>{
  ...
  const activeProposal=await env.DB.prepare("SELECT id,status FROM distribution_proposals WHERE status IN ('applied','published') ORDER BY created_at DESC LIMIT 1").first<{id:string;status:string}>();if(activeProposal)return json({error:"Existe uma distribuição aplicada ou publicada. Reverta-a antes de alterar preferências."},409);
...
async function buildDistributionCheck(env:Env){
 const [classes,students,destinations,activeProposal]=await Promise.all([
  env.DB.prepare("SELECT id,status FROM classes ORDER BY id").all<{id:number;status:string}>(),
  ...
  env.DB.prepare("SELECT id,status FROM distribution_proposals WHERE status IN ('applied','published') ORDER BY created_at DESC LIMIT 1").first<{id:string;status:string}>()
 ]);
 ...
 for(const klass of classes.results){if(["draft","reopened"].includes(klass.status))issues.push({severity:"blocker",code:"TURMA_NAO_SUBMETIDA",message:`A Turma ${klass.id} ainda não foi submetida.`,classId:klass.id});if(!(counts.get(klass.id)||0))issues.push({severity:"blocker",code:"TURMA_VAZIA",message:`A Turma ${klass.id} não tem estudantes.`,classId:klass.id})}
 let automaticStays=0,exceptionalPending=0;
 for(const student of students.results){
  const identity={classId:student.class_id,studentId:student.id,studentName:student.full_name,studentNumber:student.student_number},ownDestinations=student.student_decision==="move"?destinationsById.get(student.id)||[]:[];
 ...
 const categories=[
  {key:"roster",label:"Turmas e estudantes",description:"Turmas submetidas, não vazias e identificadores válidos.",codes:["SEM_TURMAS","SEM_ESTUDANTES","TURMA_NAO_SUBMETIDA","TURMA_VAZIA","NUMERO_INVALIDO"]},
  {key:"preferences",label:"Preferências",description:"Decisões e destinos ativos, únicos e ordenados.",codes:["PREFERENCIAS_EM_FALTA","DESTINO_INVALIDO","ORDEM_PREFERENCIAS_INVALIDA"]},
...
const DISTRIBUTION_ENGINE_VERSION="5.0.0";
...
async function handleDistributionProposals(request:Request,env:Env,user:CurrentUser,action:string):Promise<Response>{
 ...
 if(action==="calculate"){
  const active=await env.DB.prepare("SELECT id,status FROM distribution_proposals WHERE status IN ('applied','published') ORDER BY created_at DESC LIMIT 1").first<{id:string;status:string}>();if(active)return json({error:"Já existe uma distribuição aplicada ou publicada. Reverta-a antes de calcular uma nova proposta."},409);
...
async function handlePlacementWorkbench(request:Request,env:Env,user:CurrentUser):Promise<Response>{
 ...
 if(request.method==="GET"){
  ...
  if(request.method==="PATCH"){
   ...
   const activeProposal=await env.DB.prepare("SELECT id FROM distribution_proposals WHERE status IN ('applied','published') LIMIT 1").first();if(activeProposal)return json({error:"Retire primeiro a publicação ou distribuição ativa antes de alterar esta validação."},409);
 ...
 if(request.method==="PUT"){
  ...
  if(activeProposal)return json({error:"Existe uma distribuição aplicada ou publicada. Reverta-a antes de alterar preferências."},409);if(student.notes?.trim()&&!reviewStatus)return json({error:"Selecione Informação válida ou Informação inválida antes de guardar."},400);
