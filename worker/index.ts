...
async function handleOwnDestinations(request:Request,env:Env,user:CurrentUser):Promise<Response>{
  const number=studentNumberFromEmail(user.email); const student=await env.DB.prepare("SELECT s.id,s.class_id,s.preference,s.notes,c.status FROM class_students s JOIN classes c ON c.id=s.class_id WHERE s.student_number=? AND s.removed_at IS NULL").bind(number).first<{id:string;class_id:number;preference:string;notes:string|null;status:string}>();
  ...
  if(request.method==="GET"){const [destinations,classes]=await Promise.all([env.DB.prepare("SELECT destination_class FROM student_destinations WHERE student_id=? ORDER BY rank").bind(student.id).all<{destination_class:number}>(),env.DB.prepare("SELECT id FROM classes ORDER BY id").all<{id:number}>()]);return json({student:{classId:student.class_id,preference:student.preference,notes:student.notes||"",destinations:destinations.results.map(row=>row.destination_class)},activeClasses:classes.results.map(row=>row.id),settings:studentSettings,serverNow:Date.now()});}
  const body=await parseJson(request),destinations=Array.isArray(body?.destinations)?body.destinations.map(Number):[],notes=typeof body?.notes==="string"?body.notes.trim().slice(0,1000):"";if(destinations.length>19||new Set(destinations).size!==destinations.length||destinations.some(id=>!Number.isInteger(id)||id<1||id>20))return json({error:"Pode indicar até 19 turmas alternativas, sem repetições."},400);
  ...
  writes.push(env.DB.prepare("INSERT INTO class_audit_log (class_id,student_id,actor_user_id,action,details,created_at) VALUES (?,?,?,'student_preference_updated',?,?)").bind(student.class_id,student.id,user.id,JSON.stringify({destinations,hasAdditionalInfo:Boolean(notes)}),now));await env.DB.batch(writes);return json({ok:true});
}
...
async function handleDistributionCheckV2(env:Env,user:CurrentUser):Promise<Response>{
 ...
 let automaticStays=0,exceptionalPending=0,invalidReferences=0;for(const student of students.results){if(!/^\d{9}$/.test(student.student_number))issues.push({severity:"blocker",code:"NUMERO_INVALIDO",message:`Número mecanográfico inválido na Turma ${student.class_id}.`,classId:student.class_id});if(student.student_decision==="move"&&!student.destination_count)issues.push({severity:"blocker",code:"PREFERENCIAS_EM_FALTA",message:`Há um estudante que pretende mudar sem preferências na Turma ${student.class_id}.`,classId:student.class_id});if(!student.student_decision)automaticStays++;if(student.notes?.trim()&&!student.exception_reviewed_at){exceptionalPending++;issues.push({severity:"blocker",code:"INFORMACAO_POR_VALIDAR",message:`Existe informação adicional por validar na Turma ${student.class_id}.`,classId:student.class_id});}}
 ...
 const visibleIssues=issues.filter((issue)=>issue.code!=="REFERENCIA_SEM_PONTO");
 return json({ready:visibleIssues.every((issue)=>issue.severity!=="blocker"),checkedAt:Date.now(),summary:{classes:classes.results.length,students:students.results.length,blockers:visibleIssues.filter((issue)=>issue.severity==="blocker").length,warnings:visibleIssues.filter((issue)=>issue.severity==="warning").length,automaticStays,exceptionalPending},issues:visibleIssues});
}

const DISTRIBUTION_ENGINE_VERSION="4.0.0";
...
async function handlePlacementWorkbench(request:Request,env:Env,user:CurrentUser):Promise<Response>{
 ...
 if(request.method==="GET"){const [students,destinations,references]=await Promise.all([env.DB.prepare("SELECT id,full_name,student_number,class_id,student_decision,preference_source,preference_admin_reason,notes,considerations,exception_points,exception_reviewed_at,exception_review_reason,additional_info_validation,additional_info_validation_note,distribution_result FROM class_students WHERE removed_at IS NULL ORDER BY class_id,full_name COLLATE NOCASE").all(),env.DB.prepare("SELECT student_id,destination_class,rank FROM student_destinations ORDER BY student_id,rank").all(),env.DB.prepare("SELECT p.student_id,p.friend_student_id,p.destination_class,f.full_name friend_name,f.student_number friend_number,f.class_id friend_class_id,f.student_decision friend_decision FROM student_friend_preferences p LEFT JOIN class_students f ON f.id=p.friend_student_id ORDER BY p.student_id,p.rank").all()]);return json({students:students.results,destinations:destinations.results,references:references.results});}
 if(request.method==="PUT"){const body=await parseJson(request),studentId=String(body?.studentId||""),reason=String(body?.reason||"").trim().slice(0,500),destinations=Array.isArray(body?.destinations)?body.destinations.map(Number):[],validationType=String(body?.validationType||""),validationNote=String(body?.validationNote||"").trim().slice(0,500),customPoints=Number(body?.customPoints||0);if(!reason)return json({error:"A justificação administrativa é obrigatória."},400);if(!["","friends_other_class","integration_bullying","other"].includes(validationType)||validationType==="other"&&(!validationNote||!Number.isInteger(customPoints)||customPoints<0||customPoints>5))return json({error:"Na opção Outro, justifique e indique entre 0 e 5 pontos."},400);if(new Set(destinations).size!==destinations.length||destinations.some(value=>!Number.isInteger(value)||value<1||value>20))return json({error:"Preferências inválidas."},400);const student=await env.DB.prepare("SELECT id,class_id,notes FROM class_students WHERE id=? AND removed_at IS NULL").bind(studentId).first<{id:string;class_id:number;notes:string|null}>();if(!student||destinations.includes(student.class_id))return json({error:"Estudante ou destinos inválidos."},400);const now=Date.now(),considerations=validationType?[validationType]:[],points=validationType==="integration_bullying"?2:validationType==="friends_other_class"?1:validationType==="other"?customPoints:0,writes=[env.DB.prepare("DELETE FROM student_destinations WHERE student_id=?").bind(studentId),env.DB.prepare("DELETE FROM student_friend_preferences WHERE student_id=?").bind(studentId),env.DB.prepare("UPDATE class_students SET preference=?,student_decision=?,decision_at=?,preference_source='admin',preference_admin_reason=?,considerations=?,exception_points=?,exception_reviewed_at=?,exception_reviewed_by=?,exception_review_reason=?,additional_info_validation=?,additional_info_validation_note=?,manual_review=0,distribution_result='pending',updated_at=? WHERE id=?").bind(destinations.length?"move":"stay",destinations.length?"move":"stay",now,reason,JSON.stringify(considerations),points,validationType?now:null,validationType?actorId:null,validationType?validationNote||reason:null,validationType||null,validationType==="other"?validationNote:null,now,studentId)];if(destinations.length){const values=destinations.map(()=>"(?,?,?,?,?)").join(","),bindings=destinations.flatMap((destination,rank)=>[studentId,destination,rank+1,actorId,now]);writes.push(env.DB.prepare(`INSERT INTO student_destinations (student_id,destination_class,rank,updated_by,updated_at) VALUES ${values}`).bind(...bindings));}writes.push(env.DB.prepare("UPDATE distribution_proposals SET invalidated_at=? WHERE invalidated_at IS NULL AND status IN ('draft','approved')").bind(now),env.DB.prepare("INSERT INTO admin_audit_log (actor_user_id,action,details,created_at) VALUES (?,'student_preferences_admin_updated',?,?)").bind(actorId,JSON.stringify({studentId,destinations,validationType,validationNote,customPoints,reason}),now));await env.DB.batch(writes);return json({ok:true});}
 return json({error:"Operação não suportada."},405);
}
