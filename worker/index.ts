...
async function handleClassesV2(request: Request, env: Env, user: CurrentUser, pathname: string): Promise<Response> {
  ...
  if(pathname==="/api/classes/import"&&request.method==="POST"){
   ...
   if(importedRows.some(row=>!Number.isInteger(row.classId)||row.classId<1||row.classId>20||!row.parsedStatus))return json({error:"Cada linha deve indicar uma turma entre 1 e 20 e codigo_estatuto N, TE, A ou O."},400);
