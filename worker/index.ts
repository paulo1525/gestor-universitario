...
async function handleOwnDestinations(request:Request,env:Env,user:CurrentUser):Promise<Response>{
  ...
  const body=await parseJson(request),allowed=["with_person","integration_bullying","other_exception"]; const destinations=Array.isArray(body?.destinations)?body.destinations.map(Number):[],notes=typeof body?.notes==="string"?body.notes.trim().slice(0,1000):"",considerations=Array.isArray(body?.considerations)?[...new Set(body.considerations.filter((value):value is string=>typeof value==="string"&&allowed.includes(value)))]:[]; if(destinations.length>19||new Set(destinations).size!==destinations.length||destinations.some((id)=>!Number.isInteger(id)||id<1||id>20))return json({error:"Pode indicar até 19 turmas alternativas, sem repetições."},400);
  const sensitive=considerations.some(value=>["integration_bullying","other_exception"].includes(value));if(notes&&!sensitive)return json({error:"A informação adicional só pode ser usada numa situação excecional."},400);
  ...
  const requestedFriends=Array.isArray(body?.friends)?body.friends.map((item)=>({studentId:String(item?.studentId||""),destinationClass:Number(item?.destinationClass)})):[];const friendIds=requestedFriends.map(item=>item.studentId),friendDestinations=requestedFriends.map(item=>item.destinationClass);if(requestedFriends.length>destinations.length||new Set(friendIds).size!==friendIds.length||new Set(friendDestinations).size!==friendDestinations.length||requestedFriends.some(item=>!item.studentId||item.studentId===student.id||!destinations.includes(item.destinationClass))||(!considerations.includes("with_person")&&requestedFriends.length))return json({error:"Indica no máximo uma pessoa distinta por turma escolhida."},400);let friends:Array<{studentId:string;destinationClass:number}>=[];
  if(friendIds.length){const placeholders=friendIds.map(()=>"?").join(","),rows=await env.DB.prepare(`SELECT id,class_id FROM class_students WHERE removed_at IS NULL AND id IN (${placeholders})`).bind(...friendIds).all<{id:string;class_id:number}>();if(rows.results.length!==friendIds.length)return json({error:"A pessoa indicada já não pertence a uma turma ativa."},409);const byId=new Map(rows.results.map(row=>[row.id,row]));if(requestedFriends.some(item=>byId.get(item.studentId)?.class_id!==item.destinationClass))return json({error:"Cada pessoa de referência tem de pertencer à turma dessa preferência."},409);friends=requestedFriends;}
  const writes=[env.DB.prepare("DELETE FROM student_destinations WHERE student_id=?").bind(student.id),env.DB.prepare("DELETE FROM student_friend_preferences WHERE student_id=?").bind(student.id),env.DB.prepare("UPDATE class_students SET preference=?,student_decision=?,decision_at=?,preference_source='student',preference_admin_reason=NULL,distribution_result='pending',notes=?,considerations=?,support_class=NULL,friend_group_code=NULL,manual_review=?,exception_points=CASE WHEN ? THEN exception_points ELSE 0 END,exception_reviewed_at=CASE WHEN ? THEN exception_reviewed_at ELSE NULL END,updated_at=? WHERE id=?").bind(destinations.length?'move':'stay',destinations.length?'move':'stay',now,notes,JSON.stringify(considerations),sensitive||notes?1:0,considerations.includes("other_exception")?1:0,considerations.includes("other_exception")?1:0,now,student.id)];
  ...
  if(friends.length){const values=friends.map(()=>"(?,?,?,?,?)").join(","),bindings=friends.flatMap(friend=>[student.id,friend.studentId,friend.destinationClass,destinations.indexOf(friend.destinationClass)+1,now]);writes.push(env.DB.prepare(`INSERT INTO student_friend_preferences (student_id,friend_student_id,destination_class,rank,updated_at) VALUES ${values}`).bind(...bindings));}
...
async function handleDistributionCheckV2(env:Env,user:CurrentUser):Promise<Response>{
 if(!canManageAll(user))return json({error:"Acesso reservado ao Núcleo de Gestão."},403);
 const [classes,students,openTickets,counts,references]=await Promise.all([env.DB.prepare("SELECT id,status FROM classes ORDER BY id").all<{id:number;status:string}>(),env.DB.prepare(`SELECT s.id,s.class_id,s.student_number,s.preference,s.student_decision,s.considerations,s.exception_reviewed_at,COUNT(d.student_id) destination_count FROM class_students s LEFT JOIN student_destinations d ON d.student_id=s.id WHERE s.removed_at IS NULL GROUP BY s.id`).all<{id:string;class_id:number;student_number:string;preference:string;student_decision:string|null;considerations:string;exception_reviewed_at:number|null;destination_count:number}>(),env.DB.prepare("SELECT id,class_id FROM class_tickets WHERE status IN ('pending','approved','open','review','information_needed')").all<{id:string;class_id:number}>(),env.DB.prepare("SELECT class_id,COUNT(*) total FROM class_students WHERE removed_at IS NULL GROUP BY class_id ORDER BY class_id").all<{class_id:number;total:number}>(),env.DB.prepare("SELECT p.student_id,p.destination_class,f.class_id,f.student_decision FROM student_friend_preferences p LEFT JOIN class_students f ON f.id=p.friend_student_id AND f.removed_at IS NULL").all<{student_id:string;destination_class:number;class_id:number|null;student_decision:string|null}>()]);
 const issues:Array<{severity:"blocker"|"warning";code:string;message:string;classId?:number}>=[];
 ...
 let automaticStays=0,exceptionalPending=0,invalidReferences=0;for(const student of students.results){if(!/^\d{9}$/.test(student.student_number))issues.push({severity:"blocker",code:"NUMERO_INVALIDO",message:`Número mecanográfico inválido na Turma ${student.class_id}.`,classId:student.class_id});if(student.student_decision==="move"&&!student.destination_count)issues.push({severity:"blocker",code:"PREFERENCIAS_EM_FALTA",message:`Há um estudante que pretende mudar sem preferências na Turma ${student.class_id}.`,classId:student.class_id});if(!student.student_decision)automaticStays++;let considerations:string[]=[];try{considerations=JSON.parse(student.considerations||"[]")}catch{}if(considerations.includes("other_exception")&&!student.exception_reviewed_at){exceptionalPending++;issues.push({severity:"blocker",code:"EXCECAO_POR_AVALIAR",message:`Existe uma situação excecional por avaliar na Turma ${student.class_id}.`,classId:student.class_id});}}
 for(const reference of references.results)if(reference.class_id!==reference.destination_class||reference.student_decision==="move"){invalidReferences++;issues.push({severity:"warning",code:"REFERENCIA_SEM_PONTO",message:`Uma pessoa de referência na Turma ${reference.destination_class} não vai permanecer nessa turma; o ponto não será contado.`,classId:reference.destination_class});}
 ...
 return json({ready:issues.every((issue)=>issue.severity!=="blocker"),checkedAt:Date.now(),summary:{classes:classes.results.length,students:students.results.length,blockers:issues.filter((issue)=>issue.severity==="blocker").length,warnings:issues.filter((issue)=>issue.severity==="warning").length,automaticStays,exceptionalPending,invalidReferences},issues});
}

const DISTRIBUTION_ENGINE_VERSION="3.0.0";
type DistributionInput={id:string;studentNumber:string;classId:number;preference:"stay"|"move";studentDecision:"stay"|"move"|null;representativePreference:"stay"|"move";preferenceSource:string;notes:string|null;considerations:string[];integrationPoints:number;exceptionPoints:number;supportClass:number|null;basePoints:number;destinations:number[];friendPreferences:Array<{friendStudentId:string;destinationClass:number;valid:boolean;rank:number}>};
async function distributionInputs(env:Env){
 const [studentRows,classRows,destinationRows,friendRows]=await Promise.all([
  env.DB.prepare("SELECT id,student_number,class_id,preference,student_decision,preference_source,notes,considerations,support_class,exception_points FROM class_students WHERE removed_at IS NULL ORDER BY id").all<{id:string;student_number:string;class_id:number;preference:"stay"|"move";student_decision:"stay"|"move"|null;preference_source:string;notes:string|null;considerations:string;support_class:number|null;exception_points:number}>(),
  env.DB.prepare("SELECT id FROM classes ORDER BY id").all<{id:number}>(),
 ...
 const students:DistributionInput[]=studentRows.results.map(row=>{const decision=row.student_decision==="move"?"move":row.student_decision==="stay"?"stay":null,destinations=decision==="move"?(destinationsById.get(row.id)||[]):[];let considerations:string[]=[];try{const parsed=JSON.parse(row.considerations||"[]");if(Array.isArray(parsed))considerations=parsed.filter((value):value is string=>typeof value==="string")}catch{}const integrationPoints=considerations.includes("integration_bullying")?2:0,exceptionPoints=considerations.includes("other_exception")?Number(row.exception_points||0):0;return {id:row.id,studentNumber:row.student_number,classId:row.class_id,preference:decision==="move"?"move":"stay",studentDecision:decision,representativePreference:row.preference,preferenceSource:decision?row.preference_source:"automatic",notes:row.notes,considerations,integrationPoints,exceptionPoints,supportClass:row.support_class,basePoints:integrationPoints+exceptionPoints,destinations,friendPreferences:friendRows.results.filter(friend=>friend.student_id===row.id).map(friend=>({friendStudentId:friend.friend_student_id,destinationClass:friend.destination_class,valid:friend.friend_class_id===friend.destination_class&&friend.friend_decision!=="move",rank:friend.rank}))};});
...
async function handleDistributionProposals(request:Request,env:Env,user:CurrentUser,action:string):Promise<Response>{
 ...
 if(action==="review"){const studentId=String(body?.studentId||""),reason=String(body?.reason||"").trim().slice(0,500);if(!reason)return json({error:"Registe a justificação da validação."},400);const changed=await env.DB.prepare("UPDATE distribution_result_reviews SET status='approved',reviewed_by=?,reviewed_at=? WHERE proposal_id=? AND student_id=? AND status='pending'").bind(actorId,now,id,studentId).run();if(!changed.meta.changes)return json({error:"Revisão pendente não encontrada."},404);await env.DB.prepare("INSERT INTO admin_audit_log (actor_user_id,action,details,created_at) VALUES (?,'distribution_result_reviewed',?,?)").bind(actorId,JSON.stringify({proposalId:id,studentId,reason}),now).run();return json({ok:true});}
 if(action==="override"){if(proposal.status!=="draft")return json({error:"Só é possível editar uma proposta em rascunho."},409);const studentId=String(body?.studentId||""),destinationClass=Number(body?.destinationClass),reason=String(body?.reason||"").trim().slice(0,500);if(!reason)return json({error:"Indique a justificação da alteração manual."},400);const parsedInput=JSON.parse(proposal.input_snapshot) as {students:DistributionInput[];classIds:number[]},student=parsedInput.students.find(item=>item.id===studentId);if(!student||(!student.destinations.includes(destinationClass)&&destinationClass!==student.classId))return json({error:"O destino manual tem de ser uma preferência aceite ou a turma de origem."},400);const results=JSON.parse(proposal.result_snapshot) as Array<Record<string,unknown>&{studentId:string;destinationClass:number}>,result=results.find(item=>item.studentId===studentId);if(!result)return json({error:"Estudante não encontrado na proposta."},404);const previousClass=result.destinationClass;result.destinationClass=destinationClass;result.rank=student.destinations.indexOf(destinationClass)+1||null;result.status=destinationClass===student.classId?(student.studentDecision==="move"?"fallback":"stayed_by_choice"):"moved";result.manualReview=false;result.manualOverride=true;const totals=new Map<number,number>((parsedInput.classIds||[]).map(classId=>[classId,0]));for(const item of results)totals.set(item.destinationClass,(totals.get(item.destinationClass)||0)+1);const sizes=[...totals.values()];if(Math.max(...sizes)-Math.min(...sizes)>3)return json({error:"A alteração deixaria as turmas com uma diferença superior a três estudantes."},409);await env.DB.batch([env.DB.prepare("UPDATE distribution_proposals SET result_snapshot=? WHERE id=?").bind(JSON.stringify(results),id),env.DB.prepare("INSERT INTO distribution_manual_overrides (proposal_id,student_id,previous_class,destination_class,reason,actor_user_id,created_at) VALUES (?,?,?,?,?,?,?) ON CONFLICT(proposal_id,student_id) DO UPDATE SET previous_class=excluded.previous_class,destination_class=excluded.destination_class,reason=excluded.reason,actor_user_id=excluded.actor_user_id,created_at=excluded.created_at").bind(id,studentId,previousClass,destinationClass,reason,actorId,now),env.DB.prepare("INSERT INTO distribution_result_reviews (proposal_id,student_id,status,reviewed_by,reviewed_at) VALUES (?,?,'approved',?,?) ON CONFLICT(proposal_id,student_id) DO UPDATE SET status='approved',reviewed_by=excluded.reviewed_by,reviewed_at=excluded.reviewed_at").bind(id,studentId,actorId,now),env.DB.prepare("INSERT INTO admin_audit_log (actor_user_id,action,details,created_at) VALUES (?,'distribution_manual_override',?,?)").bind(actorId,JSON.stringify({proposalId:id,studentId,previousClass,destinationClass,reason}),now)]);return json({ok:true});}
 if(action==="rollback"){if(proposal.status==="rolled_back")return json({ok:true,status:"rolled_back",alreadyRolledBack:true});if(proposal.status!=="applied")return json({error:"Só uma proposta aplicada pode ser revertida."},409);const parsed=JSON.parse(proposal.input_snapshot) as {students:Array<{id:string;classId:number}>}|Array<{id:string;classId:number}>,input=Array.isArray(parsed)?parsed:parsed.students;await env.DB.batch([...input.map(student=>env.DB.prepare("UPDATE class_students SET class_id=?,distribution_result='pending',updated_at=? WHERE id=?").bind(student.classId,now,student.id)),env.DB.prepare("UPDATE distribution_proposals SET status='rolled_back',rolled_back_at=? WHERE id=?").bind(now,id),env.DB.prepare("INSERT INTO admin_audit_log (actor_user_id,action,details,created_at) VALUES (?,'distribution_rolled_back',?,?)").bind(actorId,JSON.stringify({proposalId:id}),now)]);return json({ok:true,status:"rolled_back"});}
 if(action==="publish"){if(proposal.status==="published")return json({ok:true,status:"published",alreadyPublished:true});if(proposal.status!=="applied")return json({error:"A proposta tem de estar aplicada antes de ser publicada."},409);await env.DB.batch([env.DB.prepare("UPDATE distribution_proposals SET status='published',published_at=?,published_by=? WHERE id=?").bind(now,actorId,id),env.DB.prepare("UPDATE classes SET status='published',updated_at=?").bind(now),env.DB.prepare("INSERT INTO admin_audit_log (actor_user_id,action,details,created_at) VALUES (?,'distribution_published',?,?)").bind(actorId,JSON.stringify({proposalId:id}),now)]);return json({ok:true,status:"published"});}
 ...
 if(action==="approve"){if(!["draft","approved"].includes(proposal.status))return json({error:"A proposta já não pode ser aprovada."},409);const pending=await env.DB.prepare("SELECT COUNT(*) total FROM distribution_result_reviews WHERE proposal_id=? AND status='pending'").bind(id).first<{total:number}>();if(pending?.total)return json({error:`Ainda existem ${pending.total} revisões manuais pendentes.`},409);await env.DB.batch([env.DB.prepare("UPDATE distribution_proposals SET status='approved',approved_by=COALESCE(approved_by,?),approved_at=COALESCE(approved_at,?) WHERE id=?").bind(actorId,now,id),env.DB.prepare("INSERT INTO admin_audit_log (actor_user_id,action,details,created_at) VALUES (?,'distribution_approved',?,?)").bind(actorId,JSON.stringify({proposalId:id,inputHash:current.hash}),now)]);return json({ok:true,status:"approved"});}
 if(action==="apply"){if(proposal.status==="applied")return json({ok:true,status:"applied",alreadyApplied:true});if(proposal.status!=="approved")return json({error:"A proposta tem de ser aprovada antes de ser aplicada."},409);const active=await env.DB.prepare("SELECT id FROM distribution_proposals WHERE status='applied' AND rolled_back_at IS NULL AND published_at IS NULL AND id<>?").bind(id).first();if(active)return json({error:"Já existe outra proposta aplicada. Reverta-a ou publique-a antes de continuar."},409);const results=JSON.parse(proposal.result_snapshot) as Array<{studentId:string;destinationClass:number;status:string;manualReview:boolean}>;const counts=new Map<number,number>();for(const result of results)counts.set(result.destinationClass,(counts.get(result.destinationClass)||0)+1);await env.DB.batch([...results.map(result=>env.DB.prepare("UPDATE class_students SET class_id=?,distribution_result=?,manual_review=?,updated_at=? WHERE id=?").bind(result.destinationClass,result.status,result.manualReview?1:0,now,result.studentId)),env.DB.prepare("UPDATE distribution_proposals SET status='applied',applied_at=? WHERE id=?").bind(now,id),env.DB.prepare("INSERT INTO admin_audit_log (actor_user_id,action,details,created_at) VALUES (?,'distribution_applied',?,?)").bind(actorId,JSON.stringify({proposalId:id,inputHash:current.hash,classCounts:Object.fromEntries(counts)}),now)]);return json({ok:true,status:"applied"});}
 return json({error:"Operação não suportada."},405);
}
...
const validationExportLabels:Record<string,string>={stay:"Ficar",move:"Mudar",with_person:"Com pessoa específica",integration_bullying:"Dificuldades graves de integração ou bullying",other_exception:"Outra situação excecional",support_first_choice:"Rede de apoio na primeira preferência",support_other_choice:"Rede de apoio noutra preferência",move_with_friends:"Permanecer com amigos",bullying_discrimination:"Bullying/discriminação/exclusão",serious_integration:"Dificuldade grave de integração"};
...
type ValidationExportStudent={id:string;full_name:string;student_number:string;class_id:number;class_status:string;preference:string;student_decision:string|null;decision_at:number|null;preference_locked_at:number;notes:string|null;considerations:string;support_class:number|null;friend_group_code:string|null;manual_review:number;distribution_result:string|null;created_at:number;updated_at:number};
type ValidationExportDestination={student_id:string;destination_class:number;rank:number};
type ValidationExportFriend={student_id:string;rank:number;destination_class:number;friend_name:string|null;friend_number:string|null;friend_class_id:number|null};
...
async function handlePlacementWorkbench(request:Request,env:Env,user:CurrentUser):Promise<Response>{
 ...
 if(request.method==="GET"){const [students,destinations,references]=await Promise.all([env.DB.prepare("SELECT id,full_name,student_number,class_id,student_decision,preference_source,preference_admin_reason,notes,considerations,exception_points,exception_reviewed_at,distribution_result FROM class_students WHERE removed_at IS NULL ORDER BY class_id,full_name COLLATE NOCASE").all(),env.DB.prepare("SELECT student_id,destination_class,rank FROM student_destinations ORDER BY student_id,rank").all(),env.DB.prepare("SELECT p.student_id,p.friend_student_id,p.destination_class,f.full_name friend_name,f.student_number friend_number,f.class_id friend_class_id,f.student_decision friend_decision FROM student_friend_preferences p LEFT JOIN class_students f ON f.id=p.friend_student_id ORDER BY p.student_id,p.rank").all()]);return json({students:students.results,destinations:destinations.results,references:references.results});}
 if(request.method==="PUT"){const body=await parseJson(request),studentId=String(body?.studentId||""),reason=String(body?.reason||"").trim().slice(0,500),destinations=Array.isArray(body?.destinations)?body.destinations.map(Number):[],notes=String(body?.notes||"").trim().slice(0,1000),considerations=Array.isArray(body?.considerations)?body.considerations.filter((value):value is string=>["with_person","integration_bullying","other_exception"].includes(String(value))):[],exceptionPoints=Number(body?.exceptionPoints||0),requestedReferences=Array.isArray(body?.references)?body.references.map(item=>({destinationClass:Number(item?.destinationClass),studentNumber:String(item?.studentNumber||"")})):[];if(!reason)return json({error:"A justificação administrativa é obrigatória."},400);if(new Set(destinations).size!==destinations.length||destinations.some(value=>!Number.isInteger(value)||value<1||value>20)||!Number.isInteger(exceptionPoints)||exceptionPoints<0||exceptionPoints>5||new Set(requestedReferences.map(item=>item.destinationClass)).size!==requestedReferences.length||requestedReferences.some(item=>!destinations.includes(item.destinationClass)||!/^\d{9}$/.test(item.studentNumber)))return json({error:"Preferências, referências ou pontuação inválidas."},400);const student=await env.DB.prepare("SELECT id,class_id FROM class_students WHERE id=? AND removed_at IS NULL").bind(studentId).first<{id:string;class_id:number}>();if(!student||destinations.includes(student.class_id))return json({error:"Estudante ou destinos inválidos."},400);const referenceStudents=[] as Array<{id:string;class_id:number}>;for(const reference of requestedReferences){const found=await env.DB.prepare("SELECT id,class_id FROM class_students WHERE student_number=? AND removed_at IS NULL").bind(reference.studentNumber).first<{id:string;class_id:number}>();if(!found||found.id===studentId||found.class_id!==reference.destinationClass)return json({error:`A referência ${reference.studentNumber} não pertence à Turma ${reference.destinationClass}.`},400);referenceStudents.push(found)}const now=Date.now(),hasOther=considerations.includes("other_exception"),writes=[env.DB.prepare("DELETE FROM student_destinations WHERE student_id=?").bind(studentId),env.DB.prepare("DELETE FROM student_friend_preferences WHERE student_id=?").bind(studentId),env.DB.prepare("UPDATE class_students SET preference=?,student_decision=?,decision_at=?,preference_source='admin',preference_admin_reason=?,notes=?,considerations=?,exception_points=?,exception_reviewed_at=?,exception_reviewed_by=?,exception_review_reason=?,distribution_result='pending',updated_at=? WHERE id=?").bind(destinations.length?"move":"stay",destinations.length?"move":"stay",now,reason,notes,JSON.stringify(considerations),hasOther?exceptionPoints:0,hasOther?now:null,hasOther?actorId:null,hasOther?reason:null,now,studentId)];if(destinations.length){const values=destinations.map(()=>"(?,?,?,?,?)").join(","),bindings=destinations.flatMap((destination,rank)=>[studentId,destination,rank+1,actorId,now]);writes.push(env.DB.prepare(`INSERT INTO student_destinations (student_id,destination_class,rank,updated_by,updated_at) VALUES ${values}`).bind(...bindings));}if(referenceStudents.length){const values=referenceStudents.map(()=>"(?,?,?,?,?)").join(","),bindings=referenceStudents.flatMap((reference,index)=>[studentId,reference.id,requestedReferences[index].destinationClass,destinations.indexOf(requestedReferences[index].destinationClass)+1,now]);writes.push(env.DB.prepare(`INSERT INTO student_friend_preferences (student_id,friend_student_id,destination_class,rank,updated_at) VALUES ${values}`).bind(...bindings));}writes.push(env.DB.prepare("UPDATE distribution_proposals SET invalidated_at=? WHERE invalidated_at IS NULL AND status IN ('draft','approved')").bind(now),env.DB.prepare("INSERT INTO admin_audit_log (actor_user_id,action,details,created_at) VALUES (?,'student_preferences_admin_updated',?,?)").bind(actorId,JSON.stringify({studentId,destinations,references:requestedReferences,considerations,exceptionPoints,reason}),now));await env.DB.batch(writes);return json({ok:true});}
 return json({error:"Operação não suportada."},405);
}
...
async function handleValidationExport(env:Env,user:CurrentUser):Promise<Response>{
 if(!canManageAll(user))return json({error:"Acesso reservado ao Núcleo de Gestão."},403);
 const [students,destinations,friends]=await Promise.all([
  env.DB.prepare(`SELECT s.id,s.full_name,s.student_number,s.class_id,c.status class_status,s.preference,s.student_decision,s.decision_at,s.preference_locked_at,s.notes,s.considerations,s.support_class,s.friend_group_code,s.manual_review,s.distribution_result,s.created_at,s.updated_at FROM class_students s JOIN classes c ON c.id=s.class_id WHERE s.removed_at IS NULL ORDER BY s.class_id,s.full_name COLLATE NOCASE,s.student_number`).all<ValidationExportStudent>(),
  ...
  env.DB.prepare("SELECT p.student_id,p.rank,p.destination_class,f.full_name friend_name,f.student_number friend_number,f.class_id friend_class_id FROM student_friend_preferences p LEFT JOIN class_students f ON f.id=p.friend_student_id ORDER BY p.student_id,p.rank").all<ValidationExportFriend>(),
 ]);
 const destinationsByStudent=new Map<string,ValidationExportDestination[]>(),friendsByStudent=new Map<string,ValidationExportFriend[]>();
 ...
 for(const row of friends.results)friendsByStudent.set(row.student_id,[...(friendsByStudent.get(row.student_id)||[]),row]);
 const headers=["ID interno","Nome completo","Número mecanográfico","Turma registada / origem","Estado da turma","Decisão do estudante","Preferência registada na formação inicial","Preferências de destino (por ordem)","Colegas indicados (por ordem)","Situações a considerar","Códigos das situações","Turma da rede de apoio","Grupo de apoio","Informação adicional / nota","Revisão manual","Resultado da distribuição","Preferência bloqueada em","Decisão submetida em","Registo criado em","Última atualização em"];
 const exportRows=students.results.map((student)=>{
  let considerations:string[]=[];try{const parsed=JSON.parse(student.considerations||"[]");if(Array.isArray(parsed))considerations=parsed.filter((value):value is string=>typeof value==="string")}catch{}
  ...
  const indicatedFriends=(friendsByStudent.get(student.id)||[]).map(row=>`${row.rank}.º: ${row.friend_name||"Pessoa removida"}${row.friend_number?` (${row.friend_number})`:""} — origem ${validationExportClass(row.friend_class_id)}; destino associado ${validationExportClass(row.destination_class)}`).join("; ");
  const values=[student.id,student.full_name,student.student_number,validationExportClass(student.class_id),validationClassStatusLabels[student.class_status]||student.class_status,validationExportLabels[student.student_decision||""]||"A aguardar decisão",validationExportLabels[student.preference]||student.preference,orderedDestinations,indicatedFriends,considerations.map(value=>validationExportLabels[value]||value).join("; "),considerations.join("; "),validationExportClass(student.support_class),student.friend_group_code||"",student.notes||"",Number(student.manual_review)?"Sim":"Não",validationResultLabels[student.distribution_result||"pending"]||student.distribution_result||"Pendente",validationExportDate(student.preference_locked_at),validationExportDate(student.decision_at),validationExportDate(student.created_at),validationExportDate(student.updated_at)];
  return values;
 });
 const exportData=exportRows;
 const widths=[18,24,16,12,16,18,24,34,48,36,24,18,18,48,14,28,22,22,22,22];
 const columns=widths.map((width,index)=>`<col min="${index+1}" max="${index+1}" width="${width}" customWidth="1"/>`).join("");
 const lastRow=exportData.length+3;
 const sheetXml=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><dimension ref="A1:T${lastRow}"/><sheetViews><sheetView workbookViewId="0" showGridLines="0"><pane ySplit="3" topLeftCell="A4" activePane="bottomLeft" state="frozen"/><selection pane="bottomLeft" activeCell="A4" sqref="A4"/></sheetView></sheetViews><sheetFormatPr defaultRowHeight="18"/><cols>${columns}</cols><sheetData>${xlsxRow(1,["Gestor Universitário — exportação do validador"],1,30)}${xlsxRow(2,[`Exportado em ${validationExportDate(Date.now())} · Dados confidenciais`],2,24)}${xlsxRow(3,headers,3,36)}${exportData.map((values,index)=>xlsxRow(index+4,values,index%2?5:4,42)).join("")}</sheetData><autoFilter ref="A3:T${lastRow}"/><mergeCells count="2"><mergeCell ref="A1:T1"/><mergeCell ref="A2:T2"/></mergeCells><pageMargins left="0.25" right="0.25" top="0.5" bottom="0.5" header="0.2" footer="0.2"/><pageSetup orientation="landscape" fitToWidth="1" fitToHeight="0"/></worksheet>`;
 ...
 return new Response(xlsxZip(files),{headers:{"content-type":"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet","content-disposition":`attachment; filename="validador-distribuicao-${new Date().toISOString().slice(0,10)}.xlsx`}});
}
...
async function routeApi(request: Request, env: Env, url: URL): Promise<Response> {
  ...
  if(pathname==="/api/admin/placements"&&["GET","PUT"].includes(request.method)){const user=await currentUser(request,env);return user?handlePlacementWorkbench(request,env,user):json({error:"Sessão inválida."},401);}
