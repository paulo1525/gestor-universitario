...
async function handleOwnDestinations(request:Request,env:Env,user:CurrentUser):Promise<Response>{
  ...
  const effectiveSupportClass=considerations.includes("support_first_choice")?(destinations[0]||null):supportClass;if(considerations.includes("support_other_choice")&&(!effectiveSupportClass||destinations.indexOf(effectiveSupportClass)<1))return json({error:"A rede de apoio noutra turma só pode ser indicada a partir da segunda preferência."},400);if(effectiveSupportClass&&!destinations.includes(effectiveSupportClass))return json({error:"A turma com rede de apoio tem de constar das suas preferências."},400);const sensitive=considerations.some(value=>["bullying_discrimination","serious_integration","other_exception"].includes(value));if(notes&&!sensitive)return json({error:"A informação adicional só pode ser usada numa situação excecional."},400);
  ...
  const requestedFriends=Array.isArray(body?.friends)?body.friends.map((item)=>String(item?.studentId||"")):[];if(requestedFriends.length>6||new Set(requestedFriends).size!==requestedFriends.length||requestedFriends.some(id=>!id||id===student.id)||(!considerations.includes("move_with_friends")&&requestedFriends.length))return json({error:"Pode associar até seis colegas distintos ao selecionar a opção de permanecer junto de amigos."},400);let friends:Array<{studentId:string;destinationClass:number}>=[];
  if(requestedFriends.length){const placeholders=requestedFriends.map(()=>"?").join(","),rows=await env.DB.prepare(`SELECT id,class_id,preference FROM class_students WHERE removed_at IS NULL AND id IN (${placeholders})`).bind(...requestedFriends).all<{id:string;class_id:number;preference:string}>();if(rows.results.length!==requestedFriends.length||rows.results.some(row=>row.preference!=="move"))return json({error:"Só pode indicar colegas que também pretendem mudar de turma."},409);const byId=new Map(rows.results.map(row=>[row.id,row]));friends=requestedFriends.map(studentId=>({studentId,destinationClass:byId.get(studentId)!.class_id}));}
...
async function handleDistributionProposals(request:Request,env:Env,user:CurrentUser,action:string):Promise<Response>{
 ...
 if(action==="calculate"){
  ...
  const destinationsById=new Map(raw.map(row=>[row.id,JSON.parse(row.destinations) as number[]]));const students=raw.map(row=>({id:row.id,classId:row.class_id,preference:row.preference,notes:row.notes,considerations:JSON.parse(row.considerations||"[]") as string[],supportClass:row.support_class,destinations:destinationsById.get(row.id)||[],friendPreferences:friendRows.results.filter(friend=>friend.student_id===row.id).map(friend=>({friendStudentId:friend.friend_student_id,destinations:destinationsById.get(friend.friend_student_id)||[],rank:friend.rank}))}));
