...
async function handleClassesV2(request: Request, env: Env, user: CurrentUser, pathname: string): Promise<Response> {
  ...
  if (pathname === "/api/classes" && request.method === "GET") {
    const result = await env.DB.prepare(`SELECT c.id,c.status,c.submitted_at,u.full_name representative,
      COUNT(s.id) students,COALESCE(SUM(s.special_status='none' AND s.student_decision='stay'),0) stays,COALESCE(SUM(s.special_status='none' AND s.student_decision='move'),0) moves
  ...
  if(pathname==="/api/classes/import"&&request.method==="POST"){
   ...
   const importedRows=rows.map((value:unknown)=>{const row=value&&typeof value==="object"?value as Record<string,unknown>:{},classId=Number(row.turma),specialStatus=studentStatusFromCode(row.codigo_estatuto);return {classId,specialStatus,fullName:row.nome,studentNumber:String(row.n_mecanografico||"")}});
   if(importedRows.some(row=>!Number.isInteger(row.classId)||row.classId<1||row.classId>20||!row.specialStatus))return json({error:"Cada linha deve indicar uma turma entre 1 e 20 e codigo_estatuto N, TE, A ou O."},400);
   const normalized=normalizeDraftStudents(importedRows.map(row=>({fullName:row.fullName,studentNumber:row.studentNumber,preference:"stay",specialStatus:row.specialStatus})));
   if(normalized.error||!normalized.students||normalized.students.length!==rows.length)return json({error:normalized.error||"O CSV contém uma linha incompleta."},400);
   ...
   const classById=new Map(classes.results.map(row=>[row.id,row])),affectedClassIds=[...new Set(importedRows.map(row=>row.classId))].sort((a,b)=>a-b);
   ...
   for(let start=0;start<normalized.students.length;start+=10){const chunk=normalized.students.slice(start,start+10),values=chunk.map(()=>"(?,?,?,?,?,?,?,?,?,?)").join(","),bindings=chunk.flatMap((student,index)=>[student.id,importedRows[start+index].classId,student.fullName,student.studentNumber,"stay",now,student.specialStatus,actorId,now,now]);writes.push(env.DB.prepare(`INSERT INTO class_students (id,class_id,full_name,student_number,preference,preference_locked_at,special_status,created_by,created_at,updated_at) VALUES ${values}`).bind(...bindings));}
   for(const classId of affectedClassIds){const classStudents=normalized.students.filter((_,index)=>importedRows[index].classId===classId);writes.push(env.DB.prepare("UPDATE classes SET status='submitted',workflow_step=3,submitted_at=COALESCE(submitted_at,?),submitted_by=COALESCE(submitted_by,?),updated_at=? WHERE id=?").bind(now,actorId,now,classId),env.DB.prepare("INSERT INTO class_audit_log (class_id,actor_user_id,action,details,created_at) VALUES (?,?,'class_csv_imported',?,?)").bind(classId,actorId,JSON.stringify({students:classStudents.length,statuses:Object.fromEntries(classStudents.map(student=>student.specialStatus).map(status=>[status,classStudents.filter(student=>student.specialStatus===status).length]))}),now));}
   writes.push(env.DB.prepare("INSERT INTO admin_audit_log (actor_user_id,action,details,created_at) VALUES (?,'classes_csv_imported',?,?)").bind(actorId,JSON.stringify({students:normalized.students.length,classIds:affectedClassIds}),now));
   await env.DB.batch(writes);return json({ok:true,imported:normalized.students.length,classes:affectedClassIds});
  }
  ...
  if (request.method === "GET" && action === "detail") {
    ...
    if (savedDraft) {
      output = (JSON.parse(savedDraft.payload) as DraftStudent[]).map((student) => ({ id:student.id,nome:student.fullName,numero:student.studentNumber,preferencia:"A aguardar decisão",locked:false,isSelf:student.studentNumber===ownNumber,destinations:[],...(canManageAll(user)?{specialStatus:isStudentSpecialStatus(student.specialStatus)?student.specialStatus:"none" as const}:{}) }));
    } else {
...
async function handleOwnDestinations(request:Request,env:Env,user:CurrentUser):Promise<Response>{
  ...
  if(request.method==="GET"&&student.special_status!=="none")return json({student:{classId:student.class_id,decision:null,submittedAt:null,notes:"",destinations:[],specialStatus:student.special_status,canSubmitPreferences:false},activeClasses:[],settings:studentSettings,serverNow:Date.now()});
  if(request.method!=="GET"&&student.special_status!=="none")return json({error:"Alunos com estatutos especiais não podem preencher, ainda, o formulário de preferências."},403);
...
async function buildDistributionCheck(env:Env){
 ...
 const issues:DistributionCheckIssue[]=[],activeClassIds=new Set(classes.results.map(row=>row.id)),destinationsById=new Map<string,typeof destinations.results>(),eligibleStudents=students.results.filter(student=>student.special_status==="none");
 ...
 if(!eligibleStudents.length)issues.push({severity:"blocker",code:"SEM_ESTUDANTES",message:"Não existem estudantes sem estatuto especial para distribuir."});
 ...
 for(const student of students.results){
  ...
  if(student.special_status!=="none")continue;
 ...
 return {result:{ready:blockers===0,checkedAt:Date.now(),summary:{classes:classes.results.length,students:eligibleStudents.length,specialStudents:students.results.length-eligibleStudents.length,blockers,warnings,automaticStays,exceptionalPending},checks,issues,simulation},input};
}
...
const DISTRIBUTION_ENGINE_VERSION="5.2.0";
...
async function handlePlacementWorkbench(request:Request,env:Env,user:CurrentUser):Promise<Response>{
 ...
 if(request.method==="GET"){
  const [students,destinations,classes]=await Promise.all([env.DB.prepare("SELECT id,full_name,student_number,class_id,student_decision,preference_source,preference_admin_reason,notes,considerations,exception_points,exception_reviewed_at,exception_review_reason,additional_info_validation,additional_info_validation_note,additional_info_review_status,distribution_result FROM class_students WHERE removed_at IS NULL AND special_status='none' ORDER BY class_id,full_name COLLATE NOCASE").all(),env.DB.prepare("SELECT student_id,destination_class,rank FROM student_destinations WHERE student_id IN (SELECT id FROM class_students WHERE removed_at IS NULL AND special_status='none') ORDER BY student_id,rank").all(),env.DB.prepare("SELECT id FROM classes ORDER BY id").all<{id:number}>()]);
  ...
  if(request.method==="PATCH"){
   ...
   const student=await env.DB.prepare("SELECT id,class_id,notes,special_status FROM class_students WHERE id=? AND removed_at IS NULL").bind(studentId).first<{id:string;class_id:number;notes:string|null;special_status:StudentSpecialStatus}>();if(student&&student.special_status!=="none")return json({error:"Alunos com estatutos especiais não entram na mesa de colocações."},403);if(!student?.notes?.trim())return json({error:"Este aluno não tem informação adicional para validar."},400);
 ...
 if(request.method==="PUT"){
  ...
  if(new Set(destinations).size!==destinations.length||destinations.some(value=>!Number.isInteger(value)))return json({error:"Preferências inválidas."},400);
    const [student,classes,activeProposal,currentDestinations]=await Promise.all([env.DB.prepare("SELECT id,class_id,student_decision,preference_admin_reason,notes,considerations,exception_points,additional_info_review_status,special_status FROM class_students WHERE id=? AND removed_at IS NULL").bind(studentId).first<{id:string;class_id:number;student_decision:string|null;preference_admin_reason:string|null;notes:string|null;considerations:string;exception_points:number;additional_info_review_status:string|null;special_status:StudentSpecialStatus}>(),env.DB.prepare("SELECT id FROM classes ORDER BY id").all<{id:number}>(),env.DB.prepare("SELECT id,status FROM distribution_proposals WHERE invalidated_at IS NULL AND (status='published' OR (status='applied' AND published_at IS NULL)) ORDER BY created_at DESC LIMIT 1").first<{id:string;status:string}>(),env.DB.prepare("SELECT destination_class FROM student_destinations WHERE student_id=? ORDER BY rank").bind(studentId).all<{destination_class:number}>()]);
  if(student&&student.special_status!=="none")return json({error:"Alunos com estatutos especiais não entram na mesa de colocações."},403);
