...
function calculateBestEffortDistribution(input:Awaited<ReturnType<typeof distributionInputs>>,seed:string){
 let lower=4,upper=Math.max(4,input.students.length);
 while(lower<upper){
  const candidate=Math.floor((lower+upper)/2);
  try{calculateDistribution(input.students,{seed,maxDifference:candidate,classIds:input.classIds});upper=candidate}catch{lower=candidate+1}
 }
 const results=calculateDistribution(input.students,{seed,maxDifference:lower,classIds:input.classIds});
...
async function handleDistributionProposals(request:Request,env:Env,user:CurrentUser,action:string):Promise<Response>{
 ...
 if(action==="calculate"||action==="calculate-exception"){
  ...
  const exceptional=action==="calculate-exception",calculationBody=exceptional?await parseJson(request):null,reason=exceptional?String(calculationBody?.reason||"").trim().slice(0,500):"",evaluation=await buildDistributionCheck(env);
  ...
  try{if(exceptional){const relaxed=calculateBestEffortDistribution(input,seed);results=relaxed.results;maxDifference=relaxed.maxDifference}else results=calculateDistribution(input.students,{seed,maxDifference:3,classIds:input.classIds})}catch(error){return json({error:error instanceof Error?error.message:"Não foi possível calcular uma distribuição válida."},409)}
  const finalDifference=distributionDifference(results,input.classIds),reviews=results.filter(result=>result.manualReview),auditAction=exceptional?"distribution_calculated_exception":"distribution_calculated",auditDetails=JSON.stringify({proposalId:id,seed,inputHash:input.hash,engineVersion:DISTRIBUTION_ENGINE_VERSION,students:input.students.length,manualReviews:reviews.length,maxDifference,finalDifference,imbalanceOverrideReason:exceptional?reason:null});
  await env.DB.batch([env.DB.prepare("UPDATE distribution_proposals SET invalidated_at=? WHERE invalidated_at IS NULL AND (status IN ('draft','approved') OR (status='applied' AND published_at IS NOT NULL))").bind(now),env.DB.prepare("INSERT INTO distribution_proposals (id,seed,status,input_snapshot,result_snapshot,input_hash,engine_version,max_difference,final_difference,imbalance_override_reason,created_by,created_at) VALUES (?,?,'draft',?,?,?,?,?,?,?,?,?)").bind(id,seed,input.snapshot,JSON.stringify(results),input.hash,DISTRIBUTION_ENGINE_VERSION,maxDifference,finalDifference,exceptional?reason:null,actorId,now),...reviews.map(result=>env.DB.prepare("INSERT INTO distribution_result_reviews (proposal_id,student_id,status) VALUES (?,?,'pending')").bind(id,result.studentId)),env.DB.prepare("INSERT INTO admin_audit_log (actor_user_id,action,details,created_at) VALUES (?,?,?,?)").bind(actorId,auditAction,auditDetails,now)]);
  return json({proposal:{id,seed,status:"draft",results,maxDifference,finalDifference,imbalanceOverrideReason:exceptional?reason:null}},201);
 }
 const body=await parseJson(request),id=String(body?.id||""),proposal=await env.DB.prepare("SELECT * FROM distribution_proposals WHERE id=?").bind(id).first<{status:string;input_snapshot:string;result_snapshot:string;input_hash:string|null;invalidated_at:number|null;max_difference:number;final_difference:number;imbalance_override_reason:string|null}>();if(!proposal)return json({error:"Proposta não encontrada."},404);const now=Date.now();if(proposal.invalidated_at)return json({error:"Esta proposta foi invalidada por um cálculo ou alteração posterior."},409);
 ...
 if(action==="override"){
  ...
  const totals=new Map<number,number>(classIds.map(classId=>[classId,0]));for(const item of results)totals.set(item.destinationClass,(totals.get(item.destinationClass)||0)+1);const sizes=[...totals.values()],finalDifference=Math.max(...sizes)-Math.min(...sizes);if(finalDifference>allowedDifference)return json({error:`A alteração deixaria as turmas com uma diferença superior ao limite de ${allowedDifference} estudantes desta proposta.`},409);
  await env.DB.batch([env.DB.prepare("UPDATE distribution_proposals SET result_snapshot=?,final_difference=? WHERE id=?").bind(JSON.stringify(results),finalDifference,id),env.DB.prepare("INSERT INTO distribution_manual_overrides (proposal_id,student_id,previous_class,destination_class,reason,actor_user_id,created_at) VALUES (?,?,?,?,?,?,?) ON CONFLICT(proposal_id,student_id) DO UPDATE SET previous_class=distribution_manual_overrides.previous_class,destination_class=excluded.destination_class,reason=excluded.reason,actor_user_id=excluded.actor_user_id,created_at=excluded.created_at").bind(id,studentId,previousClass,destinationClass,auditReason,actorId,now),env.DB.prepare("INSERT INTO distribution_result_reviews (proposal_id,student_id,status,reviewed_by,reviewed_at) VALUES (?,?,'approved',?,?) ON CONFLICT(proposal_id,student_id) DO UPDATE SET status='approved',reviewed_by=excluded.reviewed_by,reviewed_at=excluded.reviewed_at").bind(id,studentId,actorId,now),env.DB.prepare("INSERT INTO admin_audit_log (actor_user_id,action,details,created_at) VALUES (?,'distribution_manual_override',?,?)").bind(actorId,JSON.stringify({proposalId:id,studentId,previousClass,destinationClass,reason:auditReason,finalDifference,allowedDifference}),now)]);return json({ok:true});
 }
 if(action==="rollback"){
...
async function handlePlacementWorkbench(request:Request,env:Env,user:CurrentUser):Promise<Response>{
 ...
 if(request.method==="GET"){
  const [students,destinations,classes]=await Promise.all([env.DB.prepare(`SELECT id,full_name,student_number,class_id,student_decision,preference_source,preference_admin_reason,notes,considerations,exception_points,exception_reviewed_at,exception_review_reason,additional_info_validation,additional_info_validation_note,additional_info_review_status,distribution_result FROM class_students WHERE removed_at IS NULL${statusFilter} ORDER BY class_id,full_name COLLATE NOCASE`).all(),env.DB.prepare(`SELECT student_id,destination_class,rank FROM student_destinations WHERE student_id IN (SELECT id FROM class_students WHERE removed_at IS NULL${statusFilter}) ORDER BY student_id,rank`).all(),env.DB.prepare("SELECT id FROM classes ORDER BY id").all<{id:number}>()]);
...
async function handleValidationExport(request:Request,env:Env,user:CurrentUser):Promise<Response>{
 if(!canManageAll(user))return json({error:"Acesso reservado ao Núcleo de Gestão."},403);
 const layout=new URL(request.url).searchParams.get("layout")==="classes"?"classes":"general";
 const specialStatusesEnabled=await isModuleEnabled(env,"classes.special_statuses");
 ...
 const exportData=exportRows.map(row=>row.values),baseSubtitle=`Exportado em ${validationExportDate(Date.now())} · Dados confidenciais · ${proposal?`Proposta ${proposal.id} (${proposal.status})`:"Sem proposta calculada"}`;
 const sheets=layout==="classes"?[...new Set(exportRows.map(row=>row.classId))].sort((a,b)=>a-b).map(classId=>({name:`Turma ${classId}`,rows:exportRows.filter(row=>row.classId===classId).map(row=>row.values),title:`Gestor Universitário — pauta de colocação da Turma ${classId}`,subtitle:`${baseSubtitle} · ${exportRows.filter(row=>row.classId===classId).length} estudantes`})):[{name:"Validador — pessoas",rows:exportData,title:"Gestor Universitário — auditoria externa das pautas de colocação",subtitle:baseSubtitle}];
 ...
 const files:Array<[string,string]>=[
  ...
  ...sheets.map((sheet,index)=>[`xl/worksheets/sheet${index+1}.xml`,xlsxValidationSheet(headers,sheet.rows,sheet.title,sheet.subtitle)] as [string,string]),
 ];
 const filename=layout==="classes"?"pautas-colocacao-por-turma":"auditoria-pautas-colocacao";
