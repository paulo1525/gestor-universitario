...
async function handleDistributionProposals(request:Request,env:Env,user:CurrentUser,action:string):Promise<Response>{
 ...
 if(action==="calculate"){
  const check=await handleDistributionCheckV2(env,user),checkBody=await check.clone().json() as {ready:boolean};if(!checkBody.ready)return json({error:"Resolva os bloqueadores do verificador antes de calcular."},409);
   const [studentRows,classRows,friendRows]=await Promise.all([env.DB.prepare(`SELECT s.id,s.class_id,s.preference,s.student_decision,s.notes,s.considerations,s.support_class,s.friend_group_code,COALESCE(json_group_array(d.destination_class) FILTER (WHERE d.destination_class IS NOT NULL),'[]') destinations FROM class_students s LEFT JOIN student_destinations d ON d.student_id=s.id WHERE s.removed_at IS NULL GROUP BY s.id`).all<{id:string;class_id:number;preference:"stay"|"move";student_decision:"stay"|"move"|null;notes:string|null;considerations:string;support_class:number|null;friend_group_code:string|null;destinations:string}>(),env.DB.prepare("SELECT id FROM classes ORDER BY id").all<{id:number}>(),env.DB.prepare("SELECT student_id,friend_student_id,destination_class,rank FROM student_friend_preferences ORDER BY student_id,rank").all<{student_id:string;friend_student_id:string;destination_class:number;rank:number}>()]),raw=studentRows.results;
   const effective=raw.map(row=>{const studentDecision:"stay"|"move"|null=row.student_decision==="move"?"move":row.student_decision==="stay"?"stay":null;const effectivePreference:"stay"|"move"=studentDecision==="move"?"move":"stay";return {...row,studentDecision,effectivePreference,destinations:studentDecision==="move"?(JSON.parse(row.destinations) as number[]):[]};});
   const destinationsById=new Map(effective.map(row=>[row.id,row.destinations]));const students=effective.map(row=>({id:row.id,classId:row.class_id,preference:row.effectivePreference,studentDecision:row.studentDecision,representativePreference:row.preference,notes:row.notes,considerations:JSON.parse(row.considerations||"[]") as string[],supportClass:row.support_class,destinations:row.destinations,friendPreferences:friendRows.results.filter(friend=>friend.student_id===row.id).map(friend=>({friendStudentId:friend.friend_student_id,friendClassId:friend.destination_class,destinations:destinationsById.get(friend.friend_student_id)||[],rank:friend.rank}))}));
