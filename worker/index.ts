...
async function handleOwnDestinations(request:Request,env:Env,user:CurrentUser):Promise<Response>{
  ...
  if(request.method==="GET"){const [destinations,classes]=await Promise.all([env.DB.prepare("SELECT destination_class FROM student_destinations WHERE student_id=? ORDER BY rank").bind(student.id).all<{destination_class:number}>(),env.DB.prepare("SELECT id FROM classes ORDER BY id").all<{id:number}>()]),settings=await classSettings(env);return json({student:{classId:student.class_id,preference:student.preference,notes:student.notes||"",destinations:destinations.results.map(row=>row.destination_class)},activeClasses:classes.results.map(row=>row.id),settings,serverNow:Date.now()});}
...
async function handleDistributionProposals(request:Request,env:Env,user:CurrentUser,action:string):Promise<Response>{
 ...
 if(action==="calculate"){
  ...
  const [studentRows,classRows]=await Promise.all([env.DB.prepare(`SELECT s.id,s.class_id,s.preference,s.notes,COALESCE(json_group_array(d.destination_class) FILTER (WHERE d.destination_class IS NOT NULL),'[]') destinations FROM class_students s LEFT JOIN student_destinations d ON d.student_id=s.id WHERE s.removed_at IS NULL GROUP BY s.id`).all<{id:string;class_id:number;preference:"stay"|"move";notes:string|null;destinations:string}>(),env.DB.prepare("SELECT id FROM classes ORDER BY id").all<{id:number}>()]),raw=studentRows.results;
