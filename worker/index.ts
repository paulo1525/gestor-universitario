...
async function handleOwnDestinations(request:Request,env:Env,user:CurrentUser):Promise<Response>{
  ...
  const requestedFriends=Array.isArray(body?.friends)?body.friends.map((item)=>String(item?.studentId||"")):[];if(requestedFriends.length>6||new Set(requestedFriends).size!==requestedFriends.length||requestedFriends.some(id=>!id||id===student.id)||(!considerations.includes("with_person")&&requestedFriends.length))return json({error:"Indica até seis pessoas distintas depois de selecionar a opção correspondente."},400);let friends:Array<{studentId:string;destinationClass:number}>=[];
  if(requestedFriends.length){const placeholders=requestedFriends.map(()=>"?").join(","),rows=await env.DB.prepare(`SELECT id,class_id FROM class_students WHERE removed_at IS NULL AND id IN (${placeholders})`).bind(...requestedFriends).all<{id:string;class_id:number}>();if(rows.results.length!==requestedFriends.length)return json({error:"A pessoa indicada já não pertence a uma turma ativa."},409);const byId=new Map(rows.results.map(row=>[row.id,row]));friends=requestedFriends.map(studentId=>({studentId,destinationClass:byId.get(studentId)!.class_id}));}
  const writes=[env.DB.prepare("DELETE FROM student_destinations WHERE student_id=?").bind(student.id),env.DB.prepare("DELETE FROM student_friend_preferences WHERE student_id=?").bind(student.id),env.DB.prepare("UPDATE class_students SET preference=?,student_decision=?,decision_at=?,distribution_result='pending',notes=?,considerations=?,support_class=NULL,friend_group_code=NULL,manual_review=?,updated_at=? WHERE id=?").bind(destinations.length?'move':'stay',destinations.length?'move':'stay',now,notes,JSON.stringify(considerations),sensitive||notes?1:0,now,student.id)];
...
async function handleDistributionCheckV2(env:Env,user:CurrentUser):Promise<Response>{
 if(!canManageAll(user))return json({error:"Acesso reservado ao Núcleo de Gestão."},403);
 const [classes,students,openTickets,counts]=await Promise.all([env.DB.prepare("SELECT id,status FROM classes ORDER BY id").all<{id:number;status:string}>(),env.DB.prepare(`SELECT s.class_id,s.student_number,s.preference,s.student_decision,COUNT(d.student_id) destination_count FROM class_students s LEFT JOIN student_destinations d ON d.student_id=s.id WHERE s.removed_at IS NULL GROUP BY s.id`).all<{class_id:number;student_number:string;preference:string;student_decision:string|null;destination_count:number}>(),env.DB.prepare("SELECT id,class_id FROM class_tickets WHERE status IN ('pending','approved','open','review','information_needed')").all<{id:string;class_id:number}>(),env.DB.prepare("SELECT class_id,COUNT(*) total FROM class_students WHERE removed_at IS NULL GROUP BY class_id ORDER BY class_id").all<{class_id:number;total:number}>()]);
 const issues:Array<{severity:"blocker"|"warning";code:string;message:string;classId?:number}>=[];
 ...
 for(const student of students.results){if(!/^\d{9}$/.test(student.student_number))issues.push({severity:"blocker",code:"NUMERO_INVALIDO",message:`Número mecanográfico inválido na Turma ${student.class_id}.`,classId:student.class_id});if(student.student_decision==="move"&&!student.destination_count)issues.push({severity:"blocker",code:"PREFERENCIAS_EM_FALTA",message:`Há um estudante que pretende mudar sem preferências na Turma ${student.class_id}.`,classId:student.class_id});}
...
const DISTRIBUTION_ENGINE_VERSION="2.0.0";
type DistributionInput={id:string;studentNumber:string;classId:number;preference:"stay"|"move";studentDecision:"stay"|"move"|null;representativePreference:"stay"|"move";notes:string|null;considerations:string[];supportClass:number|null;destinations:number[];friendPreferences:Array<{friendStudentId:string;friendClassId:number;destinations:number[];rank:number}>};
async function distributionInputs(env:Env){
 const [studentRows,classRows,destinationRows,friendRows]=await Promise.all([
  env.DB.prepare("SELECT id,student_number,class_id,preference,student_decision,notes,considerations,support_class FROM class_students WHERE removed_at IS NULL ORDER BY id").all<{id:string;student_number:string;class_id:number;preference:"stay"|"move";student_decision:"stay"|"move"|null;notes:string|null;considerations:string;support_class:number|null}>(),
  ...
  env.DB.prepare("SELECT student_id,friend_student_id,destination_class,rank FROM student_friend_preferences ORDER BY student_id,rank").all<{student_id:string;friend_student_id:string;destination_class:number;rank:number}>()
 ]);
 ...
 const students:DistributionInput[]=studentRows.results.map(row=>{const decision=row.student_decision==="move"?"move":row.student_decision==="stay"?"stay":null,destinations=decision==="move"?(destinationsById.get(row.id)||[]):[];let considerations:string[]=[];try{const parsed=JSON.parse(row.considerations||"[]");if(Array.isArray(parsed))considerations=parsed.filter((value):value is string=>typeof value==="string")}catch{}return {id:row.id,studentNumber:row.student_number,classId:row.class_id,preference:decision==="move"?"move":"stay",studentDecision:decision,representativePreference:row.preference,notes:row.notes,considerations,supportClass:row.support_class,destinations,friendPreferences:friendRows.results.filter(friend=>friend.student_id===row.id).map(friend=>({friendStudentId:friend.friend_student_id,friendClassId:friend.destination_class,destinations:destinationsById.get(friend.friend_student_id)||[],rank:friend.rank}))};});
...
async function handleDistributionProposals(request:Request,env:Env,user:CurrentUser,action:string):Promise<Response>{
 ...
 if(action==="review"){const studentId=String(body?.studentId||"");const changed=await env.DB.prepare("UPDATE distribution_result_reviews SET status='approved',reviewed_by=?,reviewed_at=? WHERE proposal_id=? AND student_id=? AND status='pending'").bind(actorId,now,id,studentId).run();if(!changed.meta.changes)return json({error:"Revisão pendente não encontrada."},404);await env.DB.prepare("INSERT INTO admin_audit_log (actor_user_id,action,details,created_at) VALUES (?,'distribution_result_reviewed',?,?)").bind(actorId,JSON.stringify({proposalId:id,studentId}),now).run();return json({ok:true});}
...
async function routeApi(request: Request, env: Env, url: URL): Promise<Response> {
  ...
  const proposalAction=pathname.match(/^\/api\/admin\/distribution-proposals\/(calculate|review|approve|apply|publish|rollback)$/);if(proposalAction&&request.method==="POST"){const user=await currentUser(request,env);return user?handleDistributionProposals(request,env,user,proposalAction[1]):json({error:"Sessão inválida."},401);}
