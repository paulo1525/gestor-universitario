...
export function isMaterialsCatalogPath(pathname: string): boolean {
  ...
  return path === "/api/material-catalog" || path === "/api/material-anki" || /^\/api\/material-catalog\/[^/]+\/(download|view|highlights)$/.test(path) || /^\/api\/material-anki\/[^/]+\/download$/.test(path) || path === "/api/material-anki/media";
}
